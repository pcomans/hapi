...
| `kitchen-tipe` | code + egyptologist | 1 | 6 | 4 | Source artifacts/OCR not committed; singleton rows admitted; row tests are selective + concurrency regression test recomputes with the production helper; missing Smendes-in-N row; `21H.06` normalized dates encode a printed contradiction; several `polity` values (Tanis, Leontopolis, etc.) are not cited from Table 3. |
...
| ~~`wikipedia-ptolemaic`~~ | code + egyptologist | 1 | 6 | 4 | **Dropped 2026-04-23** — coverage redundant with pharaoh.se + Leprohon Ch X chunk 14; fixing the Rule-1 provenance gap wasn't worth the effort. 3 Ptolemaic queens unique to this source (Cleopatra III, Cleopatra V, Berenice IV) scheduled for D&H Ch 5; Cleopatra VII and Berenice III are already in Leprohon at `leprohon-33.14` / `leprohon-33.12`. |
