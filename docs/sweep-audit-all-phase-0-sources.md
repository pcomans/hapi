...
**Dropped 2026-04-23 (actioned after the sweep):** `holbl-2001-argead` (Argead bridge — pharaoh.se already covers Alexander III, Philip III Arrhidaeus, Alexander IV with full Beckerath titulary; the tracked source never reached `main` after PR #74 was closed); `wikipedia-ptolemaic` (Ptolemaic cross-reference — superseded by pharaoh.se + Leprohon Ch X chunk 14; its Rule-1 provenance gap was no longer worth fixing given the redundancy). See `docs/mvp-tasks.md` and `docs/handoff-sweep-audit-2026.md` for the scope decisions.
...
|---|---|---|
| Book-transcription source (Baud, D&H, HKW, Kitchen, Leprohon, PM Theban, Ryholt, Shaw) | YES | YES |
| Web-scrape source (pharaoh-se) | YES | YES (for ruler-attestation correctness) |
