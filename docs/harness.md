...
Pipeline and web tables live in the same Postgres database but in separate Postgres schemas (namespaces): `catalog.*` (owned by Alembic/SQLAlchemy) and `web.*` (owned by Drizzle). The `catalog` schema contains artifact data, fetch-run metadata, raw museum history, derived current raw snapshots, and fuzzy match reviews. The `web` schema contains app-specific tables like users, settings, and saved searches.
...
- `test_structure.py` — structural tests enforcing every step of the museum addition playbook: ingest assets, normalize mapper, normalize asset, fixtures (minimum 3), mapper tests, raw history/current tables, source docs, license entry, Dagster registration, mapper protocol compliance (correct source enum), and sync_search dependency wiring
...
| Fetch-run audit log | `catalog.fetch_runs` | Defines which ingest execution produced the current raw snapshot |
| Raw source history | `catalog.raw_{museum}_history` | Append-only record of what each museum said over time |
| Current raw snapshot | `catalog.raw_{museum}_current` | Latest successful source snapshot used by normalization |
