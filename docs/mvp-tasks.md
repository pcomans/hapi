...
- 🟡 **Porter-Moss Vol I (Theban Necropolis — KV/QV/TT tombs)** → `sources/porter-moss-theban-necropolis/reconciled.jsonl`. Transcribe the tomb index with occupant, dynasty, and location for KV1–KV65, selected QV, and TT1–TT400. Scope is Theban only — Memphite material needs Vol III (next task). Follows the default derived-extract path per the playbook's "Rights policy" section: PDFs stay in `proprietary/books/`, only the reconciled structured extraction is committed. Moss's tomb *descriptions* (iconography, epigraphy prose) are deliberately out of scope. Under the project's working posture, tomb-number and occupant data are extracted as facts rather than expressive content; the source `README.md` records the derived-extract basis AND the Griffith Institute licence terms under which the scans are used.
  ...
  - 🟡 **Chunk 5 in progress (this PR): KV62 Tutʿankhamun (standalone, single-row)** (PM I.2 § I.A, printed p.569–570 for the headword block; the full PM entry runs through p.586 but that's all body prose — room-by-room scene catalogues and object-by-object finds — which is emphatically out of scope for the tomb-row extractor). Per the user's direction on chunk 5 scope: **tomb-row granularity is sufficient** for the museum-data-join use case; per-chamber sub-structure would inflate the schema past what the downstream enrichment needs. The chunk-5 file is trimmed to physical p.111–112 so agents cannot wander into the body. Row captures: `occupant_name="Tutʿankhamun"` (PM-verbatim, ayin preserved per chunk-2 KV19 precedent), `notes_from_pm` with the `1st ed. 58` cross-reference and the `Excavated by Carnarvon and Carter.` clause PM prints in the ribbon, `source_citation.page=569`. The `Workmen's Rest-houses` and `Graffiti` sub-sections that follow KV62 are NOT individual tomb rows — they're sub-section prose attached to § I.A and out of scope.
