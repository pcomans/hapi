...
- ~~**Hölbl Argead bridge (Alexander → Ptolemy I)**~~ ✅ — 3-row extraction of the opening three reign banners of Hölbl's Appendix (physical PDF pp. 349–351, printed pp. 320–322) in `sources/holbl-2001-argead/reconciled.jsonl`: Alexander the Great (332–323), Philip III Arrhidaios (323–317), Alexander IV (317–310). The 332–310 BCE bridge span absent from both HKW 2006 and pharaoh.se. Pipeline: Claude Code subagent OCR → three parallel Claude Code subagent extractors → deterministic majority-vote merge. All three agents converged unanimously on every structured field; only prose `note` cells differed by wording, so no separate LLM-reviewer pass was run. **Interregnum (308–306 BCE) and Ptolemy I (305/4→) banners on physical p. 351 are deliberately out of scope** — pharaoh.se covers the Ptolemaic dynasty and duplicating here would create Phase-A drift. Human sign-off pending. Per ADR-017.
