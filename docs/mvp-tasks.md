...
- ~~**Beckerath 1999 Handbuch (cross-reference / fallback)**~~ — **Dropped from Phase 0; defer to Phase A fill-in if gaps surface.** Verified 2026-04-19 that pharaoh.se is already Beckerath-saturated: 326 of 381 pharaoh.se rows carry at least one titulary element, and **305 of those 326 rows (93%) cite Beckerath's *Handbuch* directly** in their source arrays. The 55 rows with no titulary break down as 22 Roman Emperors (outside Beckerath's Egyptian-name scope), ~15 SIP rulers already covered by Ryholt 1997, and ~18 scattered cases where no source has titulary attested. A whole-Handbuch transcription would duplicate the 93% and add titulary for at most 5–10 kings. That is not an efficient Phase-0 source — it is Phase-A curation work that can reach into Beckerath specifically for gap-fill rulers if/when `rulers.json` construction surfaces silent-pharaoh.se entries. Re-validate this decision before each Phase-A enrichment pass.
