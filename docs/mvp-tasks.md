...
- ~~**Hölbl Argead bridge (Alexander → Ptolemy I)**~~ ✅ — 3-row transcription of the Argead-rubric block from Hölbl 2001 *A History of the Ptolemaic Empire*, Appendix *Overview of the events discussed in the history of the Ptolemaic kingdom* (physical PDF pp. 348–351, printed pp. 318–321) in `sources/holbl-2001-argead/reconciled.jsonl`. Covers the three Argead-dynasty rulers of Egypt — Alexander the Great (332–323), Philip III Arrhidaios (323–317, Ptolemy as Satrap), Alexander IV (317–310/09, Ptolemy as Satrap) — closing the gap between HKW 2006's king-list coverage (which stops at Alexander) and the Ptolemaic authorities `sources/pharaoh-se/` and `sources/wikipedia-ptolemaic/` (which start at Ptolemy I Soter's kingship in 306 BCE). The 310/09–306 INTERREGNUM is explicitly excluded (not an Argead ruler; a genuine residual gap deferred to Phase A). Schema carries `holbl_id`, anglicised name, `alt_names` (Macedonian numbering + Latinised forms), `start_bce` / `end_bce`, `approximate` flag for the `310/09` split-year hedge, `polity: "Argead"`, `notes_from_holbl`, and `pdf_pages` citation. Pipeline: Claude Code subagent OCR → three parallel extraction passes → deterministic majority-vote merge → `fix_rows.py` for two egyptologist-reviewer spot corrections on `notes_from_holbl` → human sign-off (pending). Per ADR-017. Main-session-extraction deviation documented in `transcribe.md` § "Model deviation". PR #PLACEHOLDER.
