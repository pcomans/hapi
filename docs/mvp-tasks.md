...
**Row-count sanity check at breakpoint.** When Phase-0 is MVP-complete, the committed-row total should be **~3,500–3,900 rows** across ~13 sources (current 2,613 + ~900–1,000 for Leprohon + ~200 for PM Vol III first chunk + ~180–220 for D&H Ch 4 + ~60 for DH Ch 5 + ~30–60 for canonical-TT core from PM I.1). If the actual number materially undershoots this at the breakpoint, that is a signal to audit coverage before calling Phase 0 done — not a rationale for deferring more sources.
