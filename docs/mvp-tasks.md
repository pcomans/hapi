...
- **Baud 1999 Old Kingdom royal family** → `sources/baud-1999-ok-royal-family/reconciled.jsonl`. Transcribe the prosopographical *Corpus* (vol. 2) from *Famille royale et pouvoir sous l'Ancien Empire égyptien* (Baud, BdE 126, IFAO 1999). Vol. 1 narrative and vol. 2 appendices A/B/C are out of scope. This is the OK analogue of Dodson-Hilton — without it, OK queen/consort coverage will be thin while NK/LP is dense, producing an uneven authority. Required, not optional. Multi-chunk (~7 PRs keyed by Baud's own numbered entries `[1]`–`[282]`); see `docs/handoff-baud-next-chunk.md` for the chunk plan, schema, and Baud-specific reviewer risks (hedge preservation, transliteration-verbatim).
  - ✅ **Chunk 1** — entries `[1]`–`[40]`, physical pp. 11–49 of vol. 2 — 40 rows in `sources/baud-1999-ok-royal-family/reconciled.jsonl`. Covers Dyn 3–6 entries alphabetically from `///-Ḥr [1]` through `ꜥnḫ-Špss-kꜣ.f [40]`. Includes flagship queens `Jnt-kꜣ.s [22]` (daughter of Djoser), `Ankhesenmeryre I [37]` and `Ankhesenmeryre II [38]` (Pépi I's wives, mothers of Merenrê and Pépi II respectively). Pipeline: 3 parallel Claude Code extraction subagents Read the sub-PDF directly (no committed OCR per the playbook's rights policy for prose sources) → deterministic majority-vote merge → transliteration normalization (`ˁ`/`ɜ` fallback codepoints → canonical `ꜥ`/`ꜣ`) + egyptologist-reviewer spot corrections applied via `fix_rows.py` → human sign-off (pending). Per ADR-017. PR #TBD.
  - 🔜 Chunks 2–7 (`[41]`–`[282]`) — separate PRs each, ~40 entries per chunk.
