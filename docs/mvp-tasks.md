...
- **Baud 1999 Old Kingdom royal family** → `sources/baud-1999-ok-royal-family/reconciled.jsonl`. Transcribe the prosopographical *Corpus* of vol. 2 of *Famille royale et pouvoir sous l'Ancien Empire égyptien* (Baud, BdE 126, IFAO 1999). This is the OK analogue of Dodson-Hilton — without it, OK queen/consort coverage will be thin while NK/LP is dense, producing an uneven authority. Required, not optional. The *Corpus* occupies vol.2 printed pp. 395–627 as 282 numbered entries `[1]`–`[282]` spanning Dyns 3–6, ordered alphabetically by Egyptian transliteration (not by dynasty) — so it lands as a multi-chunk source across ~7 PRs per `docs/handoff-baud-next-chunk.md`. Narrative chapters of vol. 1 and vol.2 Appendices A/B/C are out of scope per the playbook rights policy and Baud's own author-flagged rejection of Appendix A.
  - 🔜 **Scaffolding (this PR — `feat/source-baud-ok-royal-family`)** — source directory, schema + prompt, `merge.py` with multi-chunk support, `fix_rows.py` with French-date parser + OK-king `king_father` authority + redirect-row normalisation, unit tests on derived-field logic, handoff doc for follow-up chunks. **No `reconciled.jsonl` yet** — the scaffolding PR did not have Task-tool access to spawn the three independent extraction subagents per ADR-017, so chunk-1 extraction is scheduled as the next PR with that access. Rationale: a single-agent extraction would fake the 3-subagent majority-vote redundancy (correlated errors), so the scaffolding lands standalone.
  - 🔜 **Chunks 1–7** — see `docs/handoff-baud-next-chunk.md`. Chunk 1 is vol.2 pp. 19–40, entries `[1]`–`[~25]`; subsequent chunks break the remaining ~257 entries into ~30-page ranges.
- **Porter-Moss Vol I (Theban Necropolis — KV/QV/TT tombs)** → `sources/porter-moss-theban-necropolis/reconciled.jsonl`. Transcribe the tomb index with occupant, dynasty, and location for KV1–KV65, selected QV, and TT1–TT400. Scope is Theban only — Memphite material needs Vol III (next task). **Before fetching or committing any raw artifact, explicitly verify the rights/licensing status for the specific PM edition or revision being used and document the basis for redistribution (or the decision to commit only a derived extract rather than the source artifact) in the source `README.md`.** Tomb-number and occupant facts are not copyrightable even where edition layout is, but the distinction must be made and recorded per source.
- **Porter-Moss Vol III (Memphis — Giza/Saqqara/Abusir)** → `sources/porter-moss-memphis/reconciled.jsonl`. Transcribe the site/tomb index from PM III (Málek's revision, 1974–1981, with ongoing Griffith Institute updates). Met/Brooklyn/Harvard hold substantial Memphite material; without this the site authority will systematically under-resolve Giza/Saqqara/Abusir provenances. Same rights-verification requirement as Vol I — record the decision in the source README.
