...
- **Dodson & Hilton royal families (NK–LP queens)** → `sources/dodson-hilton-queens/reconciled.jsonl`. Transcribe the index/tables from *The Complete Royal Families of Ancient Egypt* (Dodson & Hilton, Thames & Hudson, 2004). Covers queens, princes, princesses, and royal genealogies not in pharaoh.se. Primary fix for the missing-queens gap from the New Kingdom onward (Nefertiti, Nefertari, Tiye, Ahmose-Nefertari, Ankhesenamun, etc.). Pair with Baud 1999 (below) for Old Kingdom royal-family coverage.
...
- **Porter-Moss Vol I (Theban Necropolis — KV/QV/TT tombs)** → `sources/porter-moss-theban-necropolis/reconciled.jsonl`. Transcribe the tomb index with occupant, dynasty, and location for KV1–KV65, selected QV, and TT1–TT400. Scope is Theban only — Memphite material needs Vol III (next task). Tomb-number facts are not copyrightable; consult the Griffith Institute's revised-edition licensing before transcribing beyond tomb indices.
- **Porter-Moss Vol III (Memphis — Giza/Saqqara/Abusir)** → `sources/porter-moss-memphis/reconciled.jsonl`. Transcribe the site/tomb index from PM III (Málek's revision, 1974–1981, with ongoing Griffith Institute updates). Met/Brooklyn/Harvard hold substantial Memphite material; without this the site authority will systematically under-resolve Giza/Saqqara/Abusir provenances. Same licensing caveats as Vol I.
- **Shaw OHAE period chapter date-ranges** → `sources/shaw-ohae-2000/reconciled.jsonl`. Transcribe the date range at the head of each chapter of *The Oxford History of Ancient Egypt* (Shaw ed., 2000). Chapter-level period boundaries only — NOT a ruler-level source for any period where Kitchen/Ryholt/Hölbl cover it in depth. Gives Predynastic start bound and sub-period spans (Amarna, Ramesside, Saite, etc.) that HKW does not enumerate.
...
- **Kitchen TIPE + post-1996 revisions** → `sources/kitchen-tipe/reconciled.jsonl`. Start with the Third Intermediate Period chronological table from *The Third Intermediate Period in Egypt (1100–650 BC)*, 3rd ed. (Kitchen, 1996) — but do NOT treat it as final. Layer in Kitchen's own 2009 reassessments from Broekman/Demarée/Kaper eds. *The Libyan Period in Egypt* (Egyptologische Uitgaven 23, NINO 2009), Aston 1989 (*JEA* 75), and Jansen-Winkeln *Inschriften der Spätzeit* (2007–2014, now the working reference for TIP royal attestation). Committing Kitchen 1996 alone would bake in a chronology the field has revised. This is a multi-source task, not a single transcription.
- **Manetho fragments for Dynasty 7** → `sources/manetho-fragments/reconciled.jsonl`. Transcribe the Dyn 7 entries from Verbrugghe & Wickersham 1996 *Berossos and Manetho* (Michigan) — the modern critical edition. Waddell 1940 Loeb is easier to access but is not what a referee will expect. Source for the scholarly convention of "Dynasty 7" as Manetho's ghost dynasty.
