...
data, which is exactly the failure mode this paragraph guards against. By that query,
at minimum: **Menkheperre** — **at least three solidly distinct bearers** (Thutmose III
(Dyn 18); the Dynasty-21 High Priest Menkheperre; and **Necho I** (Dyn 24/26, prenomen
`Menkheperre` in Kitchen / `Men kheper Ra` in pharaoh-se)) **and two further contested
bearers** (**Ini**, Dyn 22, whom Beckerath gives `Men-cheper-rê (?)` — flagged
uncertain; and **Piye**, to whom Leprohon assigns Menkheperre while Kitchen gives
Piankhy *Usimare/Sneferre* instead — a disagreement *within* the committed sources)
— the contestation is itself part of what the universe must represent; **Nebmaatre**
...
- **Corroborate-or-escalate acceptance.** A matcher-derived `hapi:same_entity_as`
  ...
  for **Early Dynastic** rulers who have no prenomen at all (Narmer, Den, …),
  **Horus-name match**, which plays the same discriminating role for Dynasties 0–4 —
