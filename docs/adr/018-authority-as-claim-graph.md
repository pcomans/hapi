...
| Reader that has loaded the manifest + applies an OWL-aware reasoner | Same as the RDFS row above, **plus** the reasoner applies the manifest's `owl:SymmetricProperty` declarations (currently on `hapi:same_entity_as` and `hapi:shares_tomb_with`) and infers the inverse direction of each symmetric edge automatically. |

Free-standing Hapi predicates (those without a declared `subPropertyOf` or OWL typing) remain opaque even with the manifest loaded and full OWL reasoning — the manifest documents their existence but doesn't relate them to CRM vocabulary.
