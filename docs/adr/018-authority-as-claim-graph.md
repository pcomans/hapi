...
4. **Citation-evidence properties (`cited_page`, `cited_pdf_page`) carried on the `P70i_is_documented_in` edge itself.** Strict CRM has no `.1` sub-property of P70 for page-level citation; the edge-property idiom is a Hapi compactness choice. The round-trip materialises the page-level documentary content as its own `:E31 Document`. Three rules govern the canonical locator identity (required for lossless round-trip):
   - **Canonical locator key.** Each per-page `:E31` node is keyed by the full `(publication, cited_page, cited_pdf_page)` tuple — NOT just `(publication, page)`. This prevents the two pathological collisions: same printed page with different PDF pages (e.g. front-matter pagination differs from printed-page numbering), and same PDF page with different printed pages (facing-page spreads). One `:E31` per distinct locator tuple, with bounded growth. The physical printed page is not itself an E31 (a printed sheet is closer to E22 Human-Made Object); what's typed as `:E31` is the propositional/documentary content carried at that page location, in the same sense that the publication-level `:E31` names the publication's documentary content, not its physical binding.
   - **Two distinct identifiers per page-level `:E31`.** Both source fields survive as separate `P1_is_identified_by → :E42 Identifier` edges, distinguished by `P2_has_type → :E55 Type`:
     - one `:E42 Identifier {value: 'p. 115'}` typed `printed-page-locator`
     - one `:E42 Identifier {value: '127'}` typed `pdf-page-locator`

     P1 has domain E1 and range E41; E42 IS-A E41, so the range conforms by subsumption.
   ...
   The original `(:Statement:E13) -[:P70i_is_documented_in]-> (:E31 Document {publication})` rewrites to point at the page-level `:E31` instead of the publication-level one. **P70i's range stays E31** — no rerouting through E73/E33 required.

   Both pathological cases now round-trip losslessly: two claims at the same printed page on different PDF pages yield two distinct `:E31` sub-documents (distinguishable by their `pdf-page-locator` identifier); two claims at the same PDF page on different printed pages yield two distinct `:E31` sub-documents (distinguishable by their `printed-page-locator` identifier). The loader specification owns the rule.
...
**Hapi free-standing predicates** (`hapi:in_dynastic_period`, the three Ruler→Site tomb predicates, `hapi:display_name`, `hapi:reign_period`, `hapi:horus_name`, `hapi:matcher_review_verdict`, `hapi:supersedes` — declared in `hapi_extension.rdf` without `rdfs:subPropertyOf` to a CRM/CRMdig parent) are **NOT** in this category. They are CIDOC extensions, not deviations — CIDOC's open extension model explicitly permits new predicates introduced via the manifest idiom (every official extension, including CRMdig itself, uses the same idiom). Their CIDOC-opacity (a reader without the manifest can't reason about them) is documented in the three-reader-mode framing and the manifest's per-predicate comments. They are project vocabulary, not conceptual departures.
