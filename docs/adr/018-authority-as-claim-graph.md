...
3. **Hapi extensions need the manifest to be interpretable.** A strict reader that has loaded `hapi_extension.rdf` alongside the CRM and CRMdig RDFS files applies its `rdfs:subClassOf` / `rdfs:subPropertyOf` declarations and interprets Hapi-extended terms (`hapi:MatcherRun`, `hapi:derived_by_run`) through their declared CRM/CRMdig parents. A strict reader that has *not* loaded the manifest retains those Hapi-namespaced triples opaquely — it sees them as unrecognised URIs and does not reason on them. Free-standing Hapi predicates (those without `subPropertyOf`, like `hapi:same_entity_as` and the tomb-context predicates) remain opaque even with the manifest loaded.
...
**On the CIDOC status of these predicates.** All four tomb predicates connect a `:Ruler` (E21 Person) directly to a `:Site` (E27 Site). They are documented in the extension manifest as **pragmatic Hapi-domain predicates**, free-standing (no `rdfs:subPropertyOf`). Three things worth noting honestly:
...
2. **Heritage practice varies on rigour.** CRMarchaeo (the official CIDOC archaeology extension) materialises excavation, stratigraphy, and interment as first-class event entities (A4 Stratigraphic Genesis, A8 Stratigraphic Unit, A9 Archaeological Excavation, etc.). Museum-catalog CIDOC implementations generally use direct shortcuts when the use-case is "show me objects associated with X" rather than "reconstruct the burial stratigraphy." Both are valid CIDOC practice. Our matching use-case is the former.

3. **Rigorous alternatives differ per predicate.** `hapi:tomb_owner` is a commissioning/intended-use relationship (CIDOC-rigorous: an E12/E65 construction event with the king as `P14_carried_out_by`). `hapi:original_burial_in` is a body/interment relationship (CIDOC-rigorous: an E5 Event with `P11_had_participant` for the Ruler and `P7_took_place_at` for the Site). `hapi:cache_context_at` is a re-interment relationship (a second E5 Event chained to the first). These aren't variations of the same rigorous chain — each requires its own.
