...
- **(c′) order-independent whole-chain validation.** At load finalisation, per matcher-claim, the chain must be **connected and acyclic**: given (a) and (b), the single check "every verdict covering this claim is reachable from the chain's root" excludes both cycles (e.g. `A→B→C→A`, where every node has one in-edge and one out-edge so neither (a) nor (b) fires, but there is no root and no tip, leaving "current verdict" undefined) and disconnected sub-chains. The check is independent of load order — verdicts may be loaded in any sequence (rebuilds from serialisations, backfills, parallel loads); in Postgres terms the `predecessor_verdict_id` FK is `DEFERRABLE INITIALLY DEFERRED` so intra-transaction ordering is unconstrained, and the reachability check runs once per touched chain at commit. Honest formation cannot create a cycle (a verdict can only name an already-existing predecessor), so (c′) is defense-in-depth against corrupted or hand-edited serialisations rather than a normal-path event.
