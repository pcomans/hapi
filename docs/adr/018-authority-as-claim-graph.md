...
3. **Derived / query-only (one):** `hapi:shares_tomb_with`, symmetric `:Ruler` ↔ `:Ruler`, not loadable as a primary claim (asserting it directly would lose the place and burial-context information the rest of the model exists to preserve; see the "free-standing predicates" section above).
