...
**On the CIDOC status of these predicates.** All four tomb predicates connect a `:Ruler` (E21 Person) directly to a `:Site` (E27 Site). They are documented in the extension manifest as **pragmatic Hapi-domain predicates**, free-standing (no `rdfs:subPropertyOf`). Two things worth noting honestly:
