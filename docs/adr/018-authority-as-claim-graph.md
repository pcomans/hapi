...
| `:Site`                 | E27 Site *or* E25 Human-Made Feature (per site-kind) | **Layered typing**, following the spec's scope notes and field practice (THANADOS / ARIADNE mortuary profile: cemetery → E27; grave/burial → feature or stratigraphic unit). Territorial sites (Valley of the Kings, Saqqara) are E27 ("pieces of land"); cut or built funerary structures (KV43, TT320) are E25 — whose scope note explicitly covers "artificial caves". The site-kind vocabulary in the registry's type catalogue routes each row to its class at load; tombs link to their containing territorial site via `P46i_forms_part_of`. Every physical `:Site` node is paired 1:1 with a `:Place:E53` node via `P156_occupies`; burial claims' P141 values are the **paired E53 Place** (which is what lets `hapi:original_burial_in` / `hapi:cache_context_at` narrow under P53 — see "Implications for matching"). If CRMarchaeo is ever vendored, E25 tomb nodes refine to A8 Stratigraphic Unit. |
...
| `:Appellation`          | E41 Appellation                | Name-shaped value entity; literal accessed via P190. Type-classified via `P2_has_type → :E55 Type` only when the kind is not already fixed by the assigning Statement's P177 predicate (see encoding convention #2's exclusion rule — an `:E41` assigned via `hapi:horus_name` carries no node-level kind tag; P177 is the single source of truth). |
...
| `:Type`                 | E55 Type                       | Two distinct roles in the data model. (i) **Predicate-type registry instances** — `:E55 Type` nodes that every E13's `P177_assigned_property_of_type` edge points at; the FK target for the registry-scoped P177 column (see "Predicate registry" section). (ii) **Controlled-vocabulary value instances** — `:E55 Type` nodes used as `P141_assigned` values when the value is a closed vocabulary term rather than a substantive entity; currently the four verdict outcomes `hapi:verdict_approved` / `_rejected` / `_retracted` / `_escalated` on `hapi:matcher_review_verdict` E13s (see "Verdict-outcome controlled vocabulary" section). Both roles are CIDOC E55 Type instances; the distinction is which P-property they're a target of (P177 → registry; P141 → vocabulary value), not anything about the class itself. |
...
**Matcher-derived claims gate shortcut emission on review verdict.** For human-attested E13s (P14 → E39 Actor) the shortcut emits unconditionally. For matcher-derived E13s (`hapi:derived_by_run → :D10`) the shortcut emits only when the **unique tip of the verdict supersession chain** for the matcher's E13 has `P141 = :E55 Type {id: 'hapi:verdict_approved'}`. The tip is the verdict-E13 that has no incoming `hapi:supersedes` edge from any other verdict-E13 covering the same matcher's E13. A matcher's E13 with no verdict at all has no tip and therefore no shortcut emission; "pending" is the absence of a verdict, not a verdict outcome — distinct from `hapi:verdict_escalated`, which is an on-record referral to the curator queue (see the verdict-outcome vocabulary). Rejected, retracted, and escalated verdict outcomes also do not cause shortcut emission. The matcher's E13 is never mutated; revisiting a verdict produces a NEW verdict-E13 that carries `hapi:supersedes` to the previous tip, making the new verdict the new tip. **Supersession is an asserted relation, never a temporal inference — rationale: recency ≠ revision.** A later verdict is not necessarily a *replacement* of an earlier one: later-in-time is equally compatible with an independent parallel opinion formed in ignorance of the earlier verdict. Deriving "current" from timestamps ("most recent wins") would silently convert sequence into supersession — the same genus of silent pick Rule 2 forbids (`most_common(1)` on a tie, "first agent wins"). The replacement relation is therefore *asserted by the process that forms the verdict* and validated structurally, never inferred from `started_at`, save order, or load order. Concretely, the `hapi:supersedes` target is assigned at **formation time** and persisted in the verdict's own Rule-13 serialisation: a reviewer run records the verdict-register snapshot it read among its `L10_had_input` files, so every predecessor reference is reconstructible from the run's persisted inputs; a curator override records the verdict the curator was shown. The loader never manufactures a supersedes edge from arrival order. Chain integrity is enforced by three constraints — canonically specified in the "Verdict-outcome controlled vocabulary" section: (a) **unique successor per predecessor**, which doubles as the **collision-refusal rule** (persisting a verdict whose named predecessor already has a successor is refused at save time, so a stale writer — e.g. a batch re-review racing a same-day curator override — is surfaced and re-decides in knowledge of the newer verdict, never silently buried); (b) **unique root per matcher-claim**; (c′) **order-independent whole-chain validation** — per matcher-claim, the loaded chain must be connected and acyclic (the root reachable from every verdict by following `hapi:supersedes` edges — the root is the chain's *sink*, since edges point newer → older), checked at load finalisation, so load order carries no semantic weight and rebuilds or out-of-order loads from serialisations are safe. `started_at` / `completed_at` remain on the reviewer's `:D10` as reproducibility metadata, never as supersession state. This is how the two-stage pipeline composes with the manifest-interop story: matcher proposals exist in the graph (as full E13 reifications) regardless of verdict; only the approved tip of the supersession chain materialises the direct edge that downstream consumers query against.
...
**CIDOC-opacity and OWL-behavior are independent.** A Hapi predicate without a declared `rdfs:subPropertyOf` remains opaque to CRM/CRMdig vocabulary regardless of OWL typing — the manifest documents its existence but doesn't relate it to any CIDOC term. OWL typing (like `owl:SymmetricProperty`) adds non-CIDOC behavior that an OWL-aware reader can use; it does not add CIDOC interpretation. `hapi:shares_tomb_with` is the current example of an OWL-typed but CIDOC-opaque predicate; `hapi:tomb_owner` is both CIDOC-opaque and OWL-untyped. (The two burial predicates `hapi:original_burial_in` / `hapi:cache_context_at` are no longer examples of CIDOC-opacity — they carry `rdfs:subPropertyOf P53_has_former_or_current_location` via the E53 place pairing and are OWL-untyped but CIDOC-interpretable.)
...
- **Derived / query-only predicates** (`hapi:shares_tomb_with`): never appear as P177 targets in the persisted graph because the loader REJECTS direct E13 reifications of them; they exist only as raw query-output triples between domain entities, materialised by the query definition from the underlying primary E13s (`hapi:original_burial_in` in this case — which DOES carry P177/E55 typing as a normal P177-target predicate; `hapi:cache_context_at` and `hapi:tomb_owner` are NOT derivation sources, see the derived-predicate discussion below). The derived edge is a projection of those underlying claims, not a fresh claim with its own provenance — provenance lives on the underlying E13s. So `hapi:shares_tomb_with` carries only `rdf:Property` + `owl:SymmetricProperty` typing, NOT `crm:E55_Type`. This avoids the contradiction of an E13 claim that has no provenance edge of its own (principle 1 requires every E13 claim to carry provenance; derived edges are not E13 claims).

Hapi predicates that don't have a clean CRM/CRMdig superproperty AND that ARE P177-target predicates in the predicate registry (`hapi:in_dynastic_period`, `hapi:tomb_owner`, `hapi:display_name`, `hapi:reign_period`, `hapi:horus_name`, `hapi:matcher_review_verdict`) live in the manifest as plain `rdf:Property` declarations punned with `crm:E55_Type` so they can serve as P177 targets, and as `:E55 Type` instances in the predicate registry. (`hapi:original_burial_in` and `hapi:cache_context_at` left this bucket when they were narrowed under `P53_has_former_or_current_location` via the E53 place pairing — see the `:Site` node-table row and "Implications for matching"; they keep the same punning as `hapi:same_entity_as`, the other narrowed P177-target predicate.) Tightening these to real `rdfs:subPropertyOf` declarations is a follow-up Egyptological + CRM-modelling cross-cut. For `hapi:matcher_review_verdict` specifically, no CRMdig affinity documentation is needed: verdict Statements are additionally typed **E17 Type Assignment** (core CRM; E17 ⊂ E13). E17's `P41_classified` is `rdfs:subPropertyOf P140_assigned_attribute_to` with range E1 (the matcher's E13 qualifies via E13 ⊂ E1) and `P42_assigned` is `rdfs:subPropertyOf P141_assigned` with range E55 — both verified in the vendored RDFS — so the uniform spine holds by entailment from the E17-specific edges and strict readers interpret verdicts natively as dated, attributed classification acts (CIDOC's own P42 example is two experts assigning conflicting types to the same object — exactly the supersession-chain structure). The registry's `crm_nearest` for this predicate is `P2_has_type`, the spec's declared shortcut of the P41i/E17/P42 path, deliberately not materialised (`emit_shortcut: false` — orthodox CRM shortcut discipline). An earlier draft documented a `crmdig:L43_annotates` affinity instead; the E17 typing retires it (L43's `D29 Annotation Object` domain never accepted an E13 subject anyway). The consumed CRMdig subset is unchanged (still D1, D7, D10, D14 + L10, L11, L23, L54). **`hapi:supersedes` is separate from this list** — it is a direct E13→E13 relation, never a P177 target, so it is declared as `rdf:Property` only (no E55_Type punning). No CRM superproperty fits the "this E13 replaces that E13" semantic either (P130 / P132 are about similarity / overlap, not replacement); it is free-standing in the same modelling sense as the P177-target predicates above, but does not appear as a `:E55 Type` instance in the predicate registry.

`hapi:shares_tomb_with` is **not** in that follow-up bucket. It is registered as a **derived / query-only predicate**: the edge encodes neither the shared Site nor the burial context (primary burial vs cache), so asserting it directly would lose the very context the rest of the model exists to preserve. The loader REJECTS direct assertions of `hapi:shares_tomb_with` from any source; the source's actual statement (e.g. "King X and Queen Y in KV46") is emitted as the corresponding `original_burial_in` claims, and `hapi:shares_tomb_with` is materialised at query time by intersecting two rulers' **primary-burial place sets — *only* `hapi:original_burial_in`**. **`hapi:cache_context_at` is NOT a derivation source**: cache co-occupancy is an accident of Dynasty-21 logistics, not a burial relationship — TT320 alone holds dozens of royal mummies spanning four centuries, and deriving tomb-sharing from it would make Ramesses II "share a tomb" with kings who died centuries before him, reintroducing exactly the conflation the three-way predicate split exists to prevent. This follows scholarly practice: the literature describes cache occupants as *reburied/cached*, never as sharing a tomb, and Porter & Moss — the committed source for tomb occupancy — flags genuine joint burial as its own category (`is_joint_burial`, `co_occupants`) distinct from intrusive and cache contexts. Cache co-occupancy remains directly queryable through `hapi:cache_context_at` ("who was cached at TT320?") — it just never phrases itself as tomb-sharing. `hapi:tomb_owner` is likewise **not** a derivation source (commissioning is not interment — a king who commissioned a tomb but was never buried there should not be inferred as "buried with" anyone subsequently interred in it). The predicate registry carries a `derived: true` flag for this case (see Predicate registry section below).
...
2. **Property-graph inlining of structural attributes beyond value literals.** Inline `:E55 Type` tags (e.g. document `kind`, identifier locator kinds), inline `:E31 Document` properties (`kind`, `citation`, `year`, `language`, `rationale`), and inline `:E31` identifying labels on `:Dynasty:E4 Period` (`number`, `beckerath_label`, `leprohon_chapter`). Round-trip mappings, per attribute kind:
   - **Type tags** (document `kind`, identifier locator kinds, dynasty-label types, etc.): `P2_has_type → :E55 Type`. The inline value must resolve 1:1 to a registered `:E55 Type` instance via the predicate registry's type catalogue (a fixed mapping table committed alongside the loader). An unregistered inline type tag fails the load. **Exclusion rule (Rule 4 — single source of truth):** a value entity whose kind is already fixed by the assigning Statement's P177 predicate carries NO node-level kind tag — e.g. an `:E41` assigned via `hapi:horus_name` or `hapi:display_name` has no `appellation_kind` property; the Statement's P177 target is the sole carrier of the kind, and the loader REJECTS a node-level kind tag on such values as a hard load error. The kind remains queryable through the reification (E41 ← P141 ← E13 → P177), and via the P177 ⊂ P2 entailment on the Statement itself. Node-level type tags are reserved for kinds that no P177-target predicate carries.
...
- **Primary P177-target free-standing predicates (currently six):** `hapi:in_dynastic_period`, `hapi:tomb_owner`, `hapi:display_name`, `hapi:reign_period`, `hapi:horus_name`, `hapi:matcher_review_verdict`. Each is dually typed as `rdf:Property` + `crm:E55_Type` (punning) so it can serve as a P177 value on E13 reifications and emit shortcut triples per the registry's `emit_shortcut` field.
...
The remaining four manifest-declared predicates (`hapi:derived_by_run rdfs:subPropertyOf crm:P15_was_influenced_by`, `hapi:same_entity_as rdfs:subPropertyOf crmdig:L54_is_same_as`, and the two burial predicates `hapi:original_burial_in` / `hapi:cache_context_at`, both `rdfs:subPropertyOf crm:P53_has_former_or_current_location` via the E53 place pairing) are NOT free-standing — they're subPropertyOf-narrowed CIDOC extensions, included here only for inventory completeness. Adding a tighter `rdfs:subPropertyOf` to any of the free-standing predicates above (when a suitable parent is identified) moves that entry out of the free-standing bucket without changing what it does in the model — exactly the move the burial predicates made.
...
1. **Every claim has a provenance edge.** No fact lives in the graph without an attribution to how the claim came to be. Two distinct provenance shapes:
   ...
   Matcher-derived claims don't carry `P70i_is_documented_in` — matcher provenance is algorithmic, not documentary; review never attaches a document anchor to the matcher's E13 itself. (Verdicts are separate E13s pointing at the matcher's E13: LLM-reviewer verdicts carry the machine shape, curator verdicts resolving escalations carry P14 + P70i per ADR-020. See the two-stage pipeline principle below and schema sketch case 4b.) Human claims always carry both P14 and P70i — page-level metadata on P70i is optional but the documentary anchor itself is required.
...
**Document granularity.** Curatorial decisions are batched: one `:E31 Document {kind: 'curator_decision_batch'}` per batch (`hapi_display_names_2026_05`, `hapi_display_names_2027_q1`, …), each carrying a `decided_at` **full UTC timestamp** (ISO 8601 with at least second precision — not a bare date, which would leave same-day batches unordered) and a free-text `rationale`. `decided_at` is UNIQUE across curator-decision-batch documents (structurally enforced — Rule 3), so the resolution rule below can never encounter a tie. When a spelling is revised, a new Document is created and the old claim stays attached to its original Document as audit trail.
...
1. Prefer the Statement whose `P14_carried_out_by` is the curatorial Group `:E74 {id: 'hapi_curatorial'}` **AND** whose `P70i_is_documented_in` document has `kind = 'curator_decision_batch'`. Among matching Statements, pick the one whose document has the most recent `decided_at` — a total order, since `decided_at` is a full UTC timestamp with a uniqueness constraint (see Document granularity above); ties are structurally impossible. Timestamp ordering is sound *here* — unlike for verdicts, where supersession must be asserted (see the recency ≠ revision rationale in the Verdict-outcome section) — because all curator-decision batches come from a **single authority** (the curatorial Group), and within one authority's own decision series, later genuinely means revision by intent.
...
//     The name-kind lives solely on the Statement's P177 target (hapi:horus_name) — the value
//     node carries NO kind tag (Rule 4; see encoding convention #2's exclusion rule).
(:Statement:E13)
...
//     The verdict is itself an E13, additionally typed E17 Type Assignment (E17 ⊂ E13): a dated,
//     attributed act of classifying the matcher's E13 with a term from a closed vocabulary. E17's
//     P41_classified ⊂ P140 (range E1 — the matcher's E13 qualifies via E13 ⊂ E1) and P42_assigned
//     ⊂ P141 (range E55), so the strict-RDF export can also read the spine edges as P41/P42 and any
//     CIDOC reader interprets the verdict natively. Its P141-value is the verdict outcome as an
//     :E55 Type drawn from the verdict-outcome controlled vocabulary (see "Verdict-outcome
...
//     from any other verdict-E13 covering the same matcher's E13). The supersedes target is
//     assigned at FORMATION time by the verdict-forming process (the reviewer run records the
//     verdict-register snapshot it read among its L10 inputs; a curator override records the
//     verdict shown to the curator) and persisted in the verdict's Rule-13 serialisation — never
//     derived by the loader from arrival order or timestamps (recency ≠ revision: a later verdict
//     may be a replacement or an independent parallel opinion; only the maker's assertion can say
//     which). Chain integrity: constraints (a) unique successor per predecessor — doubling as the
//     collision-refusal rule: a save whose named predecessor already has a successor is REFUSED,
//     so a stale writer re-decides in knowledge of the ruling that beat it; (b) unique root per
//     matcher-claim; (c′) order-independent whole-chain validation (connected + acyclic per
//     claim, checked at load finalisation — load order carries no semantic weight). Verdict rows
//     are append-only. Canonical specification: §"Verdict-outcome controlled vocabulary".
//     started_at / completed_at remain on the :D10 reviewer-run as informational
...
//     L10/L11 contract.
(:Statement:E13:E17_Type_Assignment verdict-claim)
  ...
  -[:P177_assigned_property_of_type]-> (:Type:E55 {
                                           ...
                                           crm_nearest: 'P2_has_type'    // the spec's declared shortcut
                                                                         // of the P41i/E17/P42 path —
...
(:Statement:E13)
  ...
  -[:P70i_is_documented_in]->          (:Document:E31 {
                                           ...
                                           decided_at: '2026-05-17T14:03:22Z'
                                       })

// Backbone reference nodes — shared across sources
(:Dynasty:E4_Period {number, beckerath_label, leprohon_chapter})
(:Site:E27          {id: 'valley_of_the_kings'})           // territorial site ("pieces of land")
(:Site:E25          {id: 'kv43'})                           // cut tomb — E25 Human-Made Feature
  -[:P46i_forms_part_of]-> (:Site:E27 {valley_of_the_kings})
  -[:P156_occupies]->      (:Place:E53 {id: 'kv43_place'})  // 1:1 paired place; burial claims'
                                                            // P141 values target THIS node
...
| `crm_nearest`         | CIDOC P-number \| CRMdig L-property \| `null` | nearest CRM-family property for interop documentation. Accepts core CIDOC CRM P-numbers AND CRMdig L-properties (e.g. `L54_is_same_as` for `hapi:same_entity_as`; `P2_has_type` for `hapi:matcher_review_verdict` — documentation-only: the spec's declared shortcut of the P41i/E17/P42 path that verdict-E17s reify, deliberately not materialised). `null` if no clean fit. |
| `is_symmetric`        | bool                       | whether the predicate is symmetric (e.g. `hapi:same_entity_as` is) |
| `derived`             | bool                       | whether the predicate is derived from other claims (loader REJECTS direct assertions) vs loadable as a primary claim from a source. `hapi:shares_tomb_with` is the first derived predicate — it's materialised at query time by intersecting two rulers' primary-burial place sets (only from `hapi:original_burial_in` — genuine joint burial, per Porter & Moss practice; NOT from `hapi:cache_context_at`, since cache co-occupancy is not tomb-sharing, and NOT from `hapi:tomb_owner`, which is commissioning rather than interment). Asserting it directly would lose the place/context information. Default `false`. |
...
The predicate `hapi:matcher_review_verdict` (P177 type on verdict-E13s, see schema sketch case 4b) takes its P141 value from a **closed four-term vocabulary**. Each term is a separately-declared `:E55 Type` instance — formally declared in the extension manifest as `rdf:type crm:E55_Type`, NOT a magic-string in prose. The loader REJECTS any verdict-E13 whose P141 value isn't one of these four IDs. Verdict statements are additionally typed **E17 Type Assignment** (E17 ⊂ E13; P41 ⊂ P140 and P42 ⊂ P141 in the vendored RDFS, so the uniform spine holds by entailment from the E17-specific edges) — a verdict is a dated, attributed classification act on the matcher's E13, natively CIDOC-interpretable; see schema sketch case 4b.
...
| `hapi:verdict_escalated` | reviewer escalated to human | the reviewer examined the candidate and is **on record** referring it to the curator queue — a dated, attributed act of deferral, structurally distinct from the *absence* of a verdict ("pending" = nobody has looked yet; "escalated" = looked and deliberately routed to a human). This makes the escalation queue rebuildable from the graph alone: queue membership = matcher-claims whose chain tip has P141 = `hapi:verdict_escalated`. No shortcut emission (tip-outcome ≠ `hapi:verdict_approved`). The resolving curator verdict (human-documentary shape: P14 + P70i, per ADR-020's escalation queue) supersedes it as a normal chain successor. |
...
Each of these four `:E55 Type` instances is declared in `pipeline/pipeline/authority/hapi_extension.rdf` as an explicit `<rdf:Description rdf:about="#verdict_..."><rdf:type rdf:resource="...E55_Type"/></rdf:Description>` entry. The supersession edge is the `hapi:supersedes` predicate (separately declared in the manifest), domain `:E13`, range `:E13`, free-standing for now — no clean CRM superproperty was identified; P132_spatiotemporally_overlaps_with and similar candidates don't fit "this assertion replaces that one" semantics.

**Predecessor assignment happens at formation time, not load time.** The `hapi:supersedes` target is chosen by the process that *forms* the verdict and persisted in the verdict's own Rule-13 serialisation: a reviewer run reads the current verdict register before re-reviewing and records that register snapshot among its `L10_had_input` files (so every predecessor reference is reconstructible from the run's persisted inputs); a curator override records the verdict the curator was shown when deciding. The loader validates the asserted structure — it never derives supersession from arrival order, timestamps, or "what the tip happened to be when the row was saved." Rationale (recency ≠ revision): a later verdict may be a deliberate replacement *or* an independent parallel opinion formed in ignorance of the earlier one; timestamps cannot distinguish the two, so treating "latest" as "current" is a silent pick in the Rule-2 sense. Supersession must be asserted by its maker and enforced structurally. **Chain integrity requires three constraints:**

- **(a) unique successor per predecessor.** Each verdict-E13 has at most one incoming `hapi:supersedes`. In Postgres terms: a UNIQUE constraint over the `predecessor_verdict_id` column. **This constraint is simultaneously the collision-refusal mechanism**: when two verdict-makers honestly race to replace the same current verdict (a batch re-review reads the register in the morning; a curator overrides one of the same claims that afternoon; the batch saves that evening), the later save of the contested verdict violates the UNIQUE constraint and is **refused** — that one verdict is flagged for re-formation against the now-current state (in knowledge of the ruling that beat it), while the rest of the batch is unaffected. No verdict is ever silently buried by a writer that never saw it. The unique index performs the compare-and-swap; no BEFORE-INSERT trigger and no serialisable-isolation requirement is needed for this.
...
- **(c′) order-independent whole-chain validation.** At load finalisation, per matcher-claim, the chain must be **connected and acyclic**: given (a) and (b), the single check "the chain's root is reachable from every verdict covering this claim" (following `hapi:supersedes` edges newer → older; the root is the chain's *sink*, with no outgoing edge) excludes both cycles (e.g. `A→B→C→A`, where every node has one in-edge and one out-edge so neither (a) nor (b) fires, but there is no root and no tip, leaving "current verdict" undefined) and disconnected sub-chains. The check is independent of load order — verdicts may be loaded in any sequence (rebuilds from serialisations, backfills, parallel loads); in Postgres terms the `predecessor_verdict_id` FK is `DEFERRABLE INITIALLY DEFERRED` so intra-transaction ordering is unconstrained, and the reachability check runs once per touched chain at commit. Honest formation cannot create a cycle (a verdict can only name an already-existing predecessor), so (c′) is defense-in-depth against corrupted or hand-edited serialisations rather than a normal-path event.

**Verdict rows are append-only.** No UPDATE or DELETE on verdict-E13s — revisiting is always a new verdict superseding the tip. A deletion could retroactively manufacture the forks and orphans constraints (a)–(c′) exist to prevent; in Postgres terms, the loader role carries INSERT/SELECT only on the verdict table.
...
- **CRM-affinity documentation as manifest annotation triples** — *deferred until the predicate-registry and loader specs are nailed down, since it changes the registry field set.* Load-bearing spec citations currently live in prose and in the registry's `crm_nearest` string field (nearest-CRM-property affinities like P2 for the verdict predicate; considered-and-rejected superproperty candidates like P130/P132 for the supersession edge). The follow-up moves them into `hapi_extension.rdf` as documentation-only annotation properties — a fourth manifest shape (d): plain `rdf:Property` declarations (a nearest-property annotation and a rejected-superproperty annotation), no inference semantics, never in instance data, objects are real CRM/CRMdig URIs. That gives them mechanical validation for free (the manifest test's URI-resolution assertion checks every referenced term against the vendored specs — Rule 3) and a single home (Rule 4: the registry's `crm_nearest` field is then dropped in favour of the manifest triple; prose keeps qualified CRM names as non-normative renderings only, with the `cidoc-crm-validator` agent as the prose gate). This is also where the free-standing-predicate tightening lands: identifying real `rdfs:subPropertyOf` parents for the current free-standing set (see the "Conceptual deviations" section) is the same Egyptological + CRM-modelling cross-cut, and each tightening either adds a `subPropertyOf` or records the rejection as an annotation triple.
...
**On the CIDOC status of these predicates.** The site layer is **layered-typed with paired places** (see the `:Site` node-table row): territorial sites are `:Site:E27`, cut/built funerary structures are `:Site:E25` (E25's scope note explicitly covers "artificial caves"; this matches field practice — THANADOS and the ARIADNE Mortuary Data Application Profile map cemetery → E27 Site and grave/burial → feature / stratigraphic unit), a tomb links to its containing territorial site via `P46i_forms_part_of`, and every physical `:Site` node is paired 1:1 with a `:Place:E53` via `P156_occupies`. Burial claims' P141 values are the paired E53 Place. On that basis the tomb predicates split by CIDOC status:

1. **Parent-narrowed (two):** `hapi:original_burial_in` and `hapi:cache_context_at` are declared `rdfs:subPropertyOf P53_has_former_or_current_location` — domain `E21 Person` sits within P53's `E18 Physical Thing` domain (E21 ⊂ E20 ⊂ E19 ⊂ E18) and range `E53 Place` is P53's exact range: a body's burial or cache location is literally a former-or-current location. (An earlier revision classified these as free-standing because they then targeted the physical E27 node, and E27 ⊄ E53 escaped P53's range; targeting the paired place dissolves that objection.) Strict CIDOC readers with the manifest loaded interpret both natively through P53. The rigorous event chain behind `cache_context_at` — the Dynasty-21 reburial itself — is core CRM `E9 Move` (`P25 moved` / `P26 moved to`; the spec's own examples for E9 are relocations), documentable later without disturbing these shortcuts.
2. **Free-standing (one):** `hapi:tomb_owner` is commissioning, not location — no P53-family superproperty applies — and remains a pragmatic Hapi-domain predicate, loadable as a primary claim.
3. **Derived / query-only (one):** `hapi:shares_tomb_with`, symmetric `:Ruler` ↔ `:Ruler`, not loadable as a primary claim, derived from **joint primary burial only** (`hapi:original_burial_in` intersection — never from cache contexts, which are reburial logistics, not tomb-sharing; see the "free-standing predicates" section above).

CRMarchaeo, the official CIDOC archaeology extension (https://cidoc-crm.org/crmarchaeo/), remains **NOT vendored** (see `pipeline/pipeline/authority/spec/README.md`) — its references here are URL pointers, not in-tree-validated conformance claims; the conformance contract is strictly the vendored CRM 7.1.3 + CRMdig 5.0 files. If a future feature needs stratigraphic rigour, CRMarchaeo gets vendored (per the README's CRMsci-carve-out pin discipline) and E25 tomb nodes refine to `A8 Stratigraphic Unit` per the mortuary profile; the Hapi predicates here and the rigorous chain coexist.

A follow-up ADR will revise ADR-009 to specify the constraint-narrowed algorithm in detail once storage tech is decided.

## Consequences

- **Phase 0 output shape changes.** Sources still produce `reconciled.jsonl`, but the downstream loader emits per-source E13 Statements, not collapsed per-entity rows. The 3-agent extraction step's job is unchanged (faithful capture of the source); per-row resolution still follows the Rule-2 decision tree (unanimity / majority / `tie-break-overrides.json` / documented policy) — the loader trusts whatever the existing pipeline emits and never re-resolves at the graph layer. **Structural-marker rows never become Person nodes.** Rows flagged `is_dynasty_marker: true` (or a source's equivalent structural-marker flag) are faithful captures of dynasty-level table entries — Beckerath's 8 markers carry his dynasty date ranges, king counts ("17 Könige"), localisation ("in Herakleopolis") and concurrency notes ("gleichzeitig mit Dynastie 15") — and a dynasty header is not a person. The loader routes them to the `:Dynasty` (E4 Period) backbone as source-attributed E13 claims: the date range becomes a cited time-span claim (P177 target `P4_has_time-span` — the registry's `id` field accepts CIDOC P-numbers for direct CRM properties), counts and notes become cited note claims. A structural test asserts that no flagged marker row ever yields an E21 Person node (Rule 3).
