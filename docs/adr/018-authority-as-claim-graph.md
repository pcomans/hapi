...
To make the manifest-loaded interop story actually work, the loader / strict-RDF export emits **two triples per claim** for any Hapi predicate that has a `rdfs:subPropertyOf` declaration (and for any other Hapi predicate downstream consumers query directly): the full E13 reification *and* a direct shortcut triple `(subject) hapi:X (value)` between the E13's P140 subject and P141 value. The reification carries provenance; the shortcut carries the queryable relationship. This dual-emission pattern is standard CIDOC practice — the CRM scope notes themselves describe analogous "shortcut" properties (e.g. P92_brought_into_existence is a documented shortcut for the longer P92 → E63 → P4 → E52 chain). The shortcut triple is what an RDFS reasoner rewrites via `subPropertyOf` inference into the parent property (e.g. `crmdig:L54_is_same_as` for `hapi:same_entity_as`); the E13 itself is not the inference target.
