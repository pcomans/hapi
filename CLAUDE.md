...
1. **No defensive programming.** Raise on errors. No silent fallbacks, no bare `try/except`, no swallowed exceptions. Loud failures surface bugs; quiet ones hide them. Error handling is added only after a module is validated against real data and stable.
2. **Deterministic enforcement over convention.** If a rule can be a test, type constraint, or CI check, it must be. A rule that exists only in markdown is a suggestion, not enforcement.
3. **Single source of truth.** Every fact lives in exactly one place. The DB schema is SQLAlchemy models. TypeScript types are generated by Drizzle introspection. Authority data is JSON files. Duplication creates drift.
4. **Tests assert values, not absence of errors.** A test that checks "it doesn't crash" is hiding bugs. Assert specific field values against known fixture data. Every fixture test class must assert all mappable fields that the fixture populates — not just the fields relevant to the test class's "theme." A fixture with 12 populated fields needs 12 assertions, not 4.
5. **Raw data is sacred.** Ingest stores museum API responses verbatim. Transformation happens in mappers, never during ingest. If the source data is wrong, we need the original to prove it.
