...
14. **Every prompt is reviewed for answer leakage before use.** Any prompt that asks an LLM to produce or judge data must be audited for leakage first — it must not contain the answer or a proxy for it: no identifiers that encode the correspondence being asked (e.g. source ids whose numbering aligns across the records being matched), no naming of the actual answer values or pairs (including in "examples", hints, or escalation guidance), and no ordering that signals the choice (present candidates under opaque, shuffled labels). A benchmark run on a leaky prompt measures the leak, not the model — such numbers are invalid and must be discarded. Audit new or changed prompts with the `prompt-auditor` agent, and encode the guard as a test wherever possible (rule 3). This rule exists because a matcher prompt that leaked source ids + named answer pairs silently inflated benchmark precision/recall.
...
1. **Trigger a Codex review.** `@codex` (the `chatgpt-codex-connector[bot]` GitHub App) is the **primary and only** automated reviewer on this repo. It does not auto-fire on PR open — it must be triggered explicitly, on PR creation *and* after every subsequent push:
   ...
   **A reviewer that answers with anything other than an actual review has not reviewed the PR.** After triggering, confirm a review actually landed (step 2). An error comment, a "connect your account" notice, a quota warning, and total silence are all the same outcome — none of them is acceptance, and none may be treated as a pending review that will arrive later.
   ...
   The bot requires a connected GitHub account (<https://chatgpt.com/codex/cloud/settings/connectors>). If it is unconnected, over quota, or unresponsive, review locally instead — the `/codex:rescue` skill, or the reviewer subagents (`code-reviewer`, `egyptologist-reviewer`, `schema-reviewer`, `cidoc-crm-validator`), which run in-session and depend on no external service. Skipping the review step is not an option.

   **Local Codex CLI note.** Codex's own bubblewrap sandbox cannot nest inside this devcontainer — every shell command it issues dies with `bwrap: Can't mount devpts`, leaving it unable to read a single file. The devcontainer is already the sandbox, so disable the inner one in `~/.codex/config.toml`. These keys are global and must appear **before** the first `[table]` header; appending them to the end of the file silently scopes them to whatever table precedes them and has no effect:
   ```toml
   sandbox_mode = "danger-full-access"
   approval_policy = "never"
   ```
   The wrapping Claude Code Bash sandbox nests just as badly, so `codex exec` also needs `dangerouslyDisableSandbox` on the Bash call.
