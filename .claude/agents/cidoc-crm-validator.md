...
5. **The PR body of any open PR that touches the ADR or manifest** — read from the review context when available; if the PR body is not available in the agent's tool surface or prompt context, flag that as a coverage gap rather than silently skipping it
...
6. **Don't fabricate lexical-form mandates.** The spec specifies *conceptual* ranges (E61 Time Primitive, E60 Number, E62 String) but does not mandate specific lexical forms like `xsd:dateTime`, ISO 8601, or any other concrete encoding. If a change claims a particular lexical serialisation is CRM-mandated, check the scope note — it almost certainly isn't, and that's the project's choice to document as a Hapi loader convention (typically as part of an entry under ADR-018's "Property-graph encoding conventions" section, since lexical form is an encoding decision, not a conceptual deviation).
...
8. **Stale parallel mentions during review-feedback rounds.** When ADR text, the extension manifest, the manifest's block-comments, the spec README, the validator agent's own brief, and the PR body all assert the same fact (e.g. the "three Ruler→Site + one symmetric Ruler↔Ruler" framing for the tomb predicates; the CRMdig consumed-subset list; the three-reader-mode interpretive behaviour — strict-no-manifest / manifest+RDFS / manifest+OWL; the predicate-registry's two-category structure with `p177_target`), fixing one without fixing the others produces silent drift. **The failure mode has recurred in every multi-round review on this PR — most recently when the registry-bifurcation fix landed but parallel mentions in §"Open schema, enforced registry", the Implications section's CI-enforcement bullet, and the Postgres-storage "Pro" bullet each drifted in a separate round and had to be caught one-at-a-time.** This is the most expensive single failure mode in this PR's review history.
   ...
   **Discipline on any review-feedback round:**
   ...
   - **Grep both directions: what you wrote AND what you replaced.** This is the specific failure mode the validator has caught most often. When the primary-text edit replaces wording A with wording B, the cascade grep typically searches for B-shaped paraphrases — but the actual stale parallel mentions usually keep A-shaped wording (because they paraphrase the *original* framing, not the new one, and the edit author has recency bias toward B and weakened recall of A). Specific cases caught in this PR's history: "two-shape" → "three-shape" extension framing (the parallel mentions kept saying "two"); "deviation" → "encoding convention" (parallel mentions kept saying "deviation"); "any new predicate" → "any new P177-target claim predicate" (parallel mentions kept the unscoped wording); "class or predicate" → "class, predicate, or controlled-vocabulary E55 Type instance" (the hard rule kept the narrow scope). Mitigation: before the cascade-grep, explicitly write down the wording you REMOVED from the primary text. Grep for that, not just the wording you added. Counts (two/three/seven), generic category names (class/predicate, relationship type, deviation), and "see the X split" style references are the highest-risk paraphrase shapes.
   - **Grep before declaring the round clean.** For every keyword AND every paraphrase / implication of the changed concept (both new and replaced), grep across all five artifacts (ADR, manifest, manifest block-comments, spec README, validator agent def). Include the PR body if it's been written.
   ...
   - **Validator-side: do the cascade grep when validating.** When validating a round that fixed a concept-cluster, don't just verify the primary text is correct — also enumerate the concept's paraphrases (both old and new wording) and confirm none of the old-wording paraphrases remain. The validator should be running the cascade grep on the invoker's behalf as part of its standard pass.
...
1. <extension> — three sub-shapes are valid: (a) parent-narrowed declarations (`rdfs:subClassOf` / `rdfs:subPropertyOf` — the standard CIDOC extension idiom, CRMdig itself uses this same mechanism); (b) free-standing `rdf:Property` / `rdfs:Class` declarations with no CRM/CRMdig parent (project vocabulary permitted by CIDOC's open extension model but intentionally CIDOC-opaque); (c) controlled-vocabulary E55 Type instances declared via `<rdf:Description><rdf:type rdf:resource=".../E55_Type"/>` (CIDOC vocabulary INSTANCES used as P141 values, NOT predicates). For (a) and (b), confirm domain/range conform; symmetric-property domain/range identical where applicable. For (c), confirm the `rdf:type` resolves to canonical `crm:E55_Type` and the URI is used only as a P141 value, never as a P177 target. All three shapes are extensions, NOT deviations and NOT encoding conventions; flag the sub-shape per term.
...
- **Three valid non-error classifications, each with its own bar.** A change can land in any of three "clean" buckets and the bars differ:
  ...
  - **Hapi extension.** Manifest-declared Hapi terms. Three sub-shapes, all valid extensions:
    - **(a) Parent-narrowed classes / predicates** — `rdfs:subClassOf` or `rdfs:subPropertyOf` declarations within the parent's domain/range. The standard CIDOC extension idiom. Strict CIDOC readers running RDFS reasoning interpret these through their declared parents.
    - **(b) Free-standing classes / predicates** — `rdf:Property` / `rdfs:Class` declarations with no CRM/CRMdig parent. Project vocabulary permitted by CIDOC's open extension model but intentionally CIDOC-opaque (a reader with the manifest loaded sees the URI's declared `rdfs:domain` / `rdfs:range` but cannot interpret it through any CRM/CRMdig term). Not an error — documented in the three-reader-mode framing.
    - **(c) Controlled-vocabulary E55 Type instances** — `<rdf:Description rdf:about="#..."><rdf:type rdf:resource=".../E55_Type"/>` declarations. These are NOT predicates and NOT free-standing project vocabulary; they are CIDOC vocabulary INSTANCES typed directly under CIDOC's E55 Type class (CIDOC-interpretable via the explicit `rdf:type` assertion). Used as P141 values, not P177 targets. Currently: the three verdict-outcome instances (`hapi:verdict_approved` / `_rejected` / `_retracted`).

    All three sub-shapes are NOT deviations, NOT encoding conventions; a separate category. When reporting, flag the specific sub-shape per term.
...
- **Unmanifested Hapi terms are hard errors.** Every `hapi:`-namespaced class, predicate, or controlled-vocabulary E55 Type instance referenced in the ADR, schema sketch, predicate registry, or any other artifact MUST appear in the project's extension manifest (`pipeline/pipeline/authority/hapi_extension.rdf`) with a declaration appropriate to its shape: (a) a sound `rdfs:subClassOf` / `rdfs:subPropertyOf` declaration for parent-narrowed extensions; (b) an explicit free-standing `rdf:Property` / `rdfs:Class` declaration (with a comment documenting that no CRM superclass/superproperty was identified) for project-vocabulary extensions; OR (c) an `<rdf:Description rdf:about="#..."><rdf:type rdf:resource=".../E55_Type"/>` declaration for controlled-vocabulary E55 Type instances. `owl:SymmetricProperty` typing is permitted on (a) and (b) where the predicate's domain equals its range per Hapi modelling policy. Terms used but absent from the manifest have no declared relationship to CRM/CRMdig and cannot be interpreted by strict readers; report them as hard errors with the manifest line that's missing.
