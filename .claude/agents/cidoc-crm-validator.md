...
- **`owl:SymmetricProperty` declarations require identical domain and range.** A symmetric property `p` such that `(a, p, b)` implies `(b, p, a)` is only well-formed when `domain == range` (or both are subsumed by the same class — typically both E1 for cross-class symmetry, or both the same narrower class like E21). A Hapi predicate typed `owl:SymmetricProperty` with `rdfs:domain X` and `rdfs:range Y` where X ≠ Y (and neither IS-A the other) is a hard error.
