...
`egyptian_titulary_kind="nomen"` but "Nephercheres" is the Greek form of the prenomen Neferkare. Should be `egyptian_titulary_kind="prenomen"`.

**P2 — Row 31.04 Chabbasch: wrong egyptian_titulary_kind**
`egyptian_titulary_kind="nomen"` but "Senem-sotep-en-ptah" has prenomen morphology (*-sotep-en-X* suffix). Should be `egyptian_titulary_kind="prenomen"`.

**Confirmed clean:** Rows 18.11–18.14 (Nofret-ete, Semench-ka-rê, Tut-anch-amun, Aja) verified against scan-107 — all correct.

**Methodology note:** The agree-but-wrong pattern (agents partially correct on different sub-fields, majority composes an incomplete value, no disagreement logged) is a real blind spot in the three-subagent extraction protocol. It surfaces most often where Beckerath's parenthetical contains a compound value (nomen + prenomen separated by comma). Future extractions should add a post-merge check: any parenthetical containing a comma in the raw scan should produce `egyptian_titulary_kind="mixed"`, not a single-kind value.
