...
- `SHA` defaults to `git rev-parse HEAD` (git-only, sandbox-safe). Any commit-ish is accepted (short SHA, branch, tag, `HEAD~N`, etc.); `monitor.sh` normalises it to the full 40-char form before seeding. (Required: GitHub's reviews API returns full `commit_id` and the filter uses string equality.)
