...
CHUNK38_TOMB_IDS: frozenset[str] = frozenset(
    {f"TT{n}" for n in range(291, 301)}
)
CHUNK39_TOMB_IDS: frozenset[str] = frozenset(
    {f"TT{n}" for n in range(301, 311)}
)
CHUNK40_TOMB_IDS: frozenset[str] = frozenset(
    {f"TT{n}" for n in range(311, 321)}
)
...
def test_is_joint_burial_flag_paired_with_co_occupants() -> None:
    ...
    # TT298 is intentionally False (`father (probably), Unnufer` — hierarchical
    # parent-child, not co-equal joint occupancy).
    # - TT291 (chunk-38): PM p.374 lists Nu + Nekhtmin coordinately as
...
def test_182_usurped_canonical_set() -> None:
    ...
    Extended chunk 34: TT257 (Neferhotep, usurped by Maḥu, Deputy in the
    mansion of Usimare-setepenre, temp. Ramesses II — PM I.1 p.342).
...
def test_182_uncertain_attribution_canonical_set() -> None:
    ...
    expected = {
        ...
        # TT325 chunk-41: PM headword `325. Possibly SMEN (from cones). Dyn. XVIII.`
        # — `Possibly` qualifies the PRIMARY OCCUPANT identification (naming Smen
        # via cone evidence is genuinely uncertain; no inscribed attribution). This
...
def test_chunk16_tt71_senenmut_shared_with_tt353() -> None:
    ...
    Shared-with TT353 (all 3 agents agree) and TT252 (back-ref added by
    CHUNK34_CORRECTIONS: TT252 Senimen reads `Parents, see tomb 71 (brother
    Senenmut)` — pairing-invariant back-reference)."""
    r = _row("TT71")
    assert r["occupant_name"] == "Senenmut"
    assert r["occupant_role"] == "Official"
    assert r["shared_with_tombs"] == ["TT252", "TT353"]
...
# Majority resolved occupant_name disagreements (TT251 Amenmosi, TT253 Khnemmosi,
# TT257 Neferhotep — agent C had +e or macron variants).
# shared_with_tombs: TT252 A+C=["TT71"], B=[] → majority ["TT71"].
# co_occupants: TT257 B=[Mahu], A+C=[] → majority [].
# is_unfinished: TT257 A=True, B+C=False → majority False.
# Agent C off-by-one page citations: TT258 page=341 (should be 342), TT260
# page=343 (should be 344) — TT257+TT258 share physical page 342. Majority
# (A+B) resolves correctly on both.
# DERIVER_OVERRIDES: TT253, TT255, TT258, TT260 attribution_certainty → `attested`
#   (`Amenophis III (?)`, `Haremhab (?)`, `Tuthmosis IV (?)`, `Tuthmosis III (?)`
#   all hedge the regnal date, not occupant identity — same class as TT239/TT241/TT249).


def test_chunk34_all_rows_present() -> None:
    """All 10 TT251-TT260 rows must be present in reconciled.jsonl."""
    actual = {r["tomb_id"] for r in _rows()} & CHUNK34_TOMB_IDS
    assert actual == CHUNK34_TOMB_IDS, sorted(CHUNK34_TOMB_IDS - actual)
...
def test_chunk34_theban_areas() -> None:
    ...
    for tid, area in expected.items():
        assert rows[tid]["theban_area"] == area, tid


def test_chunk34_tt251_amenmosi_early_tuthmosis_iii() -> None:
    """TT251 — Amenmosi, Royal scribe. Temp. early Tuthmosis III. Sh. ʿAbd el-Qurna. p.336.
    Tie-break: notes_from_pm (1/1/1). A had complete father clause but `Amuin` OCR;
    B had `Amun` but truncated to `Nesu-...`; C truncated father clause entirely.
    Pin: `Amun` spelling + complete `Nesu-t` father name."""
    r = _row("TT251")
    assert r["occupant_name"] == "Amenmosi"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "early Tuthmosis III" in r["notes_from_pm"]
    assert "Nesu-t" in r["notes_from_pm"]
    assert "magazine of Amun" in r["notes_from_pm"]
    assert r["theban_area"] == "Sh. ʿAbd el-Qurna"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 336


def test_chunk34_tt252_senimen_nurse_godswife() -> None:
    """TT252 — Senimen, Steward, Nurse of the god's wife. Temp. Hatshepsut. p.337.
    shared_with_tombs=["TT71"] — PM `Parents, see tomb 71 (brother Senenmut)`.
    Tie-break: notes_from_pm. B missing shared_with_tombs (minority []); A+C agree."""
    r = _row("TT252")
    assert r["occupant_name"] == "Senimen"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["co_occupants"] == []
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Hatshepsut" in r["notes_from_pm"]
    assert "tomb 71" in r["notes_from_pm"]
    assert "Senenmut" in r["notes_from_pm"]
    assert "Senemiʿoḥ" in r["notes_from_pm"]
    assert r["theban_area"] == "Sh. ʿAbd el-Qurna"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 337


def test_chunk34_tt253_khnemmosi_granary_amun() -> None:
    """TT253 — Khnemmosi, Scribe, Counter of the grain. Temp. Amenophis III (?). p.338.
    Tie-break: notes_from_pm. Wife `Tannfer` (double-n, from source verbatim).
    DERIVER_OVERRIDE: attribution_certainty=attested — `Amenophis III (?)` hedges
    regnal date, not occupant identity."""
    r = _row("TT253")
    assert r["occupant_name"] == "Khnemmosi"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["co_occupants"] == []
...
def test_chunk34_tt255_roy_royal_scribe_haremhab() -> None:
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Haremhab (?)" in r["notes_from_pm"]
...
def test_chunk34_tt256_nebenkemet_fanbearer() -> None:
    """TT256 — Nebenkemet, Overseer of the cabinet, Fanbearer. Temp. Amenophis II. p.341.
    Tie-break: notes_from_pm. Wife `Ryu` (A clean, no trailing ellipsis)."""
    r = _row("TT256")
    assert r["occupant_name"] == "Nebenkemet"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["co_occupants"] == []
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Fanbearer" in r["notes_from_pm"]
    assert "Amenophis II" in r["notes_from_pm"]
...
def test_chunk34_tt257_neferhotep_usurped_by_mahu() -> None:
    ...
    but tomb itself is not uninscribed as a whole).
    Tie-break: notes_from_pm. `Maḥu` diacritic (source headword MAḤU), parents+wife+L.D. cite."""
    r = _row("TT257")
    assert r["occupant_name"] == "Neferhotep"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    ...
    assert r["is_usurped"] is True
    assert "Tuthmosis IV" in r["notes_from_pm"]
    assert "Maḥu" in r["notes_from_pm"]
    assert "Ramesseum" in r["notes_from_pm"]
    assert "Piay" in r["notes_from_pm"]
    assert "Tawert" in r["notes_from_pm"]
...
def test_chunk34_tt259_hori_warb_priest_ramesside() -> None:
    """TT259 — Hori, Warb-priest. Ramesside. Sh. ʿAbd el-Qurna. p.343.
    Tie-break: notes_from_pm. Parents: Ḥuy (underdot-Ḥ resolved) + Beketptaḥ (unhyphenated).
    Wife: Mutemwia."""
    r = _row("TT259")
    ...
    assert "House of Gold" in r["notes_from_pm"]
    assert "Ramesside" in r["notes_from_pm"]
    assert "Ḥuy" in r["notes_from_pm"]
    assert "Beketptaḥ" in r["notes_from_pm"]
    assert "Mutemwia" in r["notes_from_pm"]
    assert r["theban_area"] == "Sh. ʿAbd el-Qurna"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 343
...
def test_chunk34_tt260_user_weigher_amun() -> None:
    ...
    DERIVER_OVERRIDE: attribution_certainty=attested — `Tuthmosis III (?)` hedges regnal date."""
    r = _row("TT260")
    assert r["occupant_name"] == "User"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
...
#   TT266: 3-way split on occupant_name (Amennakit/Amennakht/Amenakht) and
#     notes_from_pm (father Buentiaf/Busentef/Busetef; wife diacritics).
#     PDF p.364 confirms AMENNAKHT and Buḳentef/Ḥenutrayunu.
#   TT267: 3-way split on notes_from_pm (mother Taarakhkau/Tarékhmani/Tarekhan;
#     wife Henumet/Henutumet/Henutmet). PDF p.365 confirms Tārekhʿan/Ḥenutmet.
#   TT268: 3-way split on notes_from_pm (lead clause `Family tomb.` vs
#     `Family tomb of Nebnakht.`; ayin on ʿAuti). PDF p.367 confirms both.
# CHUNK35_CORRECTIONS:
#   TT261 occupant_name: ayin restored → `Khaʿemweset` (PDF p.362 `KHA<EMWĒSET`).
...
# DERIVER_OVERRIDE: TT262 attribution_certainty=attested — `Tuthmosis III (?)`
#   hedges the regnal date, not occupant identity (anonymous tomb).
# shared_with_tombs: TT265 → ["TT215"] (majority A+B+C); TT266 → ["TT219"]
#   (majority A+C; `names in tomb 219` genealogical reference).


def test_chunk35_all_rows_present() -> None:
    """All 10 TT261-TT270 rows must be present in reconciled.jsonl."""
    actual = {r["tomb_id"] for r in _rows()} & CHUNK35_TOMB_IDS
    assert actual == CHUNK35_TOMB_IDS, sorted(CHUNK35_TOMB_IDS - actual)
...
def test_chunk35_theban_areas() -> None:
    ...
    expected = {
        ...
        "TT268": "Deir el-Medina",
        "TT269": "Sh. ʿAbd el-Qurna",
        "TT270": "Qurnet Muraʿi",
    }
    rows = {r["tomb_id"]: r for r in _rows() if r["tomb_id"] in expected}
    for tid, area in expected.items():
...
def test_chunk35_tt261_khaemweset_warb_priest() -> None:
    ...
    r = _row("TT261")
    assert r["occupant_name"] == "Khaʿemweset"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["co_occupants"] == []
...
def test_chunk35_tt263_piay_scribe_granary() -> None:
    """TT263 — Piay, Scribe of the granary in the Temple of Amun. Temp. Ramesses II.
    Sh. ʿAbd el-Qurna. p.345. Wife, Webekht."""
    r = _row("TT263")
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "granary" in r["notes_from_pm"]
    assert "Ramesseum" in r["notes_from_pm"]
    assert "Webekht" in r["notes_from_pm"]
    assert r["theban_area"] == "Sh. ʿAbd el-Qurna"
    assert r["dynasty"] is None
...
def test_chunk35_tt265_amenemopet_burial_chamber() -> None:
    """TT265 — Amenemopet, burial chamber of TT215. Dyn. XIX. Deir el-Medina. p.346.
    shared_with_tombs=["TT215"] — PM `(See tomb 215, which is the Chapel.)`.
    TT215 has the reciprocal back-ref ["TT265"] established in chunk-30."""
    r = _row("TT265")
    assert r["occupant_name"] == "Amenemopet"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["co_occupants"] == []
    assert r["shared_with_tombs"] == ["TT215"]
    assert r["is_joint_burial"] is False
    assert r["is_uninscribed"] is False
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "tomb 215" in r["notes_from_pm"]
    assert "Chapel" in r["notes_from_pm"]
    assert "Dyn. XIX" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 346


def test_chunk35_tt266_amennakht_chief_craftsman() -> None:
    """TT266 — Amennakht, Chief craftsman. Dyn. XIX. Deir el-Medina. p.346.
    Tie-break: occupant_name (PDF p.364 `AMENNAKHT`) and notes_from_pm
    (father `Buḳentef` underdot-ḳ; wife `Ḥenutrayunu` underdot-Ḥ).
    shared_with_tombs=[] — CHUNK35_CORRECTIONS zeros out majority A+C ["TT219"]:
    `names in tomb 219` is a one-way genealogical name-record reference,
    not a physical sharing; symmetry test rejects the asymmetric relation."""
    r = _row("TT266")
    assert r["occupant_name"] == "Amennakht"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Chief craftsman" in r["notes_from_pm"]
    assert "Place of Truth" in r["notes_from_pm"]
    assert "Buḳentef" in r["notes_from_pm"]
    assert "tomb 219" in r["notes_from_pm"]
    assert "Ḥenutrayunu" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 346


def test_chunk35_tt267_hay_officer_workmen() -> None:
    """TT267 — Hay, Officer of the workmen in the Place of Truth. Dyn. XX.
    Deir el-Medina. p.347. Tie-break: notes_from_pm (mother `Tārekhʿan` with ayin;
    wife `Ḥenutmet` underdot-Ḥ). PDF p.365 confirms both forms."""
    r = _row("TT267")
    assert r["occupant_name"] == "Hay"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["co_occupants"] == []
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Officer of the workmen" in r["notes_from_pm"]
    assert "House of Gold" in r["notes_from_pm"]
    assert "Dyn. XX" in r["notes_from_pm"]
    assert "L. D. Text" in r["notes_from_pm"]
    assert "Tārekhʿan" in r["notes_from_pm"]
    assert "Ḥenutmet" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 347


def test_chunk35_tt268_nebnakht_family_tomb() -> None:
    """TT268 — Nebnakht, family tomb, Servant in the Place of Truth. Dyn. XIX.
    Deir el-Medina. p.349. Tie-break: notes_from_pm (PM-verbatim lead
    `Family tomb of Nebnakht.` + ayin on `ʿAuti`). PDF p.367 confirms both."""
    r = _row("TT268")
    assert r["occupant_name"] == "Nebnakht"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["co_occupants"] == []
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Family tomb of Nebnakht" in r["notes_from_pm"]
    assert "Servant in the Place of Truth" in r["notes_from_pm"]
    assert "Dyn. XIX" in r["notes_from_pm"]
    assert "ʿAuti" in r["notes_from_pm"]
    assert "Turin Mus. Sup. 6044" in r["notes_from_pm"]
    assert "Thay" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 349


def test_chunk35_tt269_anonymous_name_lost() -> None:
    """TT269 — anonymous, Name lost. Ramesside. Sh. ʿAbd el-Qurna. p.349.
    CHUNK35_CORRECTIONS: occupant_role null → `Unknown` (null-name rule)."""
    r = _row("TT269")
    assert r["occupant_name"] is None
    assert r["occupant_role"] == "Unknown"
...
def test_chunk35_tt270_amenemwia_warb_priest_ptah_sokari() -> None:
    """TT270 — Amenemwia, warb-priest, Lector of Ptaḥ-Sokari. Dyn. XIX.
    Qurnet Muraʿi. p.350. Destroyed.
    CHUNK35_CORRECTIONS: `Ptah-Sokari` → `Ptaḥ-Sokari` (PDF p.368 underdot-ḥ,
    notes_from_pm verbatim-preserve policy)."""
    r = _row("TT270")
    assert r["occupant_name"] == "Amenemwia"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["co_occupants"] == []
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Ptaḥ-Sokari" in r["notes_from_pm"]
    assert "Dyn. XIX" in r["notes_from_pm"]
    assert "Destroyed" in r["notes_from_pm"]
    assert r["theban_area"] == "Qurnet Muraʿi"
    assert r["dynasty"] is None
...
# ---------------------------------------------------------------------------
# Chunk 38: TT291-TT300
# ---------------------------------------------------------------------------
# CHUNK38_CORRECTIONS applied post-merge:
#   TT292 notes_from_pm: He-nekhu → Ḥenekhu (underdot-Ḥ restored; spurious
#     hyphen removed). EGYPTOLOGIST REVIEW REQUIRED for exact printed form.
#   TT295 occupant_name: Dhutmosi → Ḏhutmosi (d-bar restored per TT32/TT205).
#   TT296 notes_from_pm: Maetmut → Maʿetmut (ayin restored; source `Ma<etmut`).
# DERIVER_OVERRIDES:
#   TT291 is_joint_burial=True (PM dual-headword Nu + Nekhtmin joint burial;
#     third joint burial after TT122/TT181; no auto-deriver for this field).
#   TT295 attribution_certainty=attested (deriver fires on regnal-date (?)
#     in `Temp. Tuthmosis IV to Amenophis III (?)`; occupant identity attested).
#   TT298 attribution_certainty=attested (deriver fires on `(probably)` in
#     `father (probably), Unnufer`; primary occupant Baki fully attested).
#   TT294 is_usurped=True via deriver auto-detect (`\busurped\b` in notes).
# Tie-breaks: 10 entries: TT291|co_occupants, TT291|notes_from_pm,
#   TT292|notes_from_pm, TT293|occupant_name, TT294|notes_from_pm,
#   TT295|occupant_name, TT295|notes_from_pm, TT298|co_occupants,
#   TT299|occupant_name, TT300|occupant_name.
# Usurpation direction: PM headword names AMENHOTEP (Amenophis III) as
#   original occupant; Roma (early Ramesside) is the usurper. Correct schema:
#   occupant_name = Amenhotep (original), is_usurped = True, co_occupants = [].


def test_chunk38_all_rows_present() -> None:
    """All 10 TT291-TT300 rows must be present in reconciled.jsonl."""
    actual = {r["tomb_id"] for r in _rows()} & CHUNK38_TOMB_IDS
    assert actual == CHUNK38_TOMB_IDS, sorted(CHUNK38_TOMB_IDS - actual)
...
def test_chunk38_theban_areas() -> None:
    ...
    by_id = {r["tomb_id"]: r for r in _rows()}
    for tid, area in expected.items():
        assert by_id[tid]["theban_area"] == area, f"{tid}: expected {area!r}"


def test_tt291_nu_and_nekhtmin() -> None:
    """TT291 — Nu + Nekhtmin, joint burial. Late Dyn. XVIII. Deir el-Medina. p.374.

    Third joint burial in PM I.1 after TT122 (chunk-21) and TT181 (chunk-27).
    co_occupants must carry Nekhtmin with name key (schema-valid).
    is_joint_burial must be True (DERIVER_OVERRIDE; agents all emitted False).
    """
    r = _row("TT291")
    assert r["occupant_name"] == "Nu"
    assert r["occupant_role"] == "Official"
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Servant in the Great Place" in r["notes_from_pm"]
    assert "Nekhtmin" in r["notes_from_pm"]
    assert "Late Dyn. XVIII" in r["notes_from_pm"]
    assert "Pia" in r["notes_from_pm"]
    assert "Mutnefert" in r["notes_from_pm"]
    assert "Khatnesut" in r["notes_from_pm"]
    assert "Min-hotep" in r["notes_from_pm"]
    assert "Nefertere" in r["notes_from_pm"]
    assert "Sekhmet" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 374


def test_tt292_peshedu() -> None:
    """TT292 — Peshedu, Servant in the Place of Truth. Temp. Sethos I–Ramesses II. p.375.

    Father name Ḥenekhu restored (OCR `l:Ie}.nekhu`); spurious hyphen removed.
    EGYPTOLOGIST REVIEW REQUIRED for exact father-name form from printed source.
    """
    r = _row("TT292")
    ...
    assert r["is_usurped"] is False
    assert "Servant in the Place of Truth" in r["notes_from_pm"]
    assert "Sethos I" in r["notes_from_pm"]
    assert "Ramesses II" in r["notes_from_pm"]
    assert "Ḥenekhu" in r["notes_from_pm"]
    assert "Brit. Mus. 262" in r["notes_from_pm"]
    assert "Makhay" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 375


def test_tt293_ramessesnakht() -> None:
    """TT293 — Raʿmessenakht, First Prophet of Amun. Temp. Ramesses IV. p.376.

    Tie-break pinned B's `Raʿmessenakht` (source `RA<MESSENAKHT`, `<` = ayin).
    A had `Ra-messenakht` (spurious hyphen), C had `Ramessenakht` (no ayin).
    """
    r = _row("TT293")
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "First prophet of Amun" in r["notes_from_pm"]
...
def test_tt294_amenhotep_usurped_by_roma() -> None:
    ...
    captured in notes_from_pm only; parallel to chunk-37 TT288 Bekenkhons).
    is_usurped=True via deriver auto-detect (`usurped` in notes_from_pm).
    Tie-break majority (A+C) correctly gives co_occupants=[]; B alone had [Roma].
    """
    r = _row("TT294")
...
def test_tt295_dhutmosi_called_paroy() -> None:
    ...
    occupant_alt_names = ["Paroy"] (PM `called PAROY` headword byname;
    majority A+C correct; B wrongly used tomb_aliases).
    attribution_certainty = attested (DERIVER_OVERRIDE; `(?)` qualifies
    regnal date, not occupant identity).
    """
    r = _row("TT295")
    assert r["occupant_name"] == "Ḏhutmosi"
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Head of the secrets in the Chest of Anubis" in r["notes_from_pm"]
...
def test_tt296_nefersekheru() -> None:
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Scribe of the divine offerings" in r["notes_from_pm"]
    assert "Southern City" in r["notes_from_pm"]
    assert "Ramesside" in r["notes_from_pm"]
...
def test_tt297_amenemopet_called_thonufer() -> None:
    """TT297 — Amenemopet called Thonufer, Scribe. Early Dyn. XVIII. ʿAsâsîf. p.379."""
    r = _row("TT297")
    assert r["occupant_name"] == "Amenemopet"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    ...
    assert "Overseer of fields" in r["notes_from_pm"]
    assert "Early Dyn. XVIII" in r["notes_from_pm"]
    assert r["theban_area"] == "ʿAsâsîf"
    assert r["dynasty"] is None
...
def test_tt298_baki_and_unnufer() -> None:
    """TT298 — Baki + father Unnufer. Ramesside. Deir el-Medina. p.379.

    Hierarchical relationship (father–son, `(probably)`); NOT a joint burial.
    co_occupants carries Unnufer (tie-break pinned A's schema-valid form with
    name key). is_joint_burial=False (correct for parent-child hierarchy).
    attribution_certainty=attested (DERIVER_OVERRIDE; `(probably)` qualifies
    father identification, not Baki's primary identity).
    """
    r = _row("TT298")
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Foreman in the Place of Truth" in r["notes_from_pm"]
    assert "father (probably)" in r["notes_from_pm"]
    assert "Unnufer" in r["notes_from_pm"]
    assert "Taysen" in r["notes_from_pm"]
    assert "tomb 213" in r["notes_from_pm"]
    assert "Maia" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 379


def test_tt299_iniherkhac() -> None:
    """TT299 — Iniherkhac, Foreman. Also owner of TT359. Deir el-Medina. p.380.

    Tie-break pinned A's `Iniherkhac` (source `INI;IERKHAC]`, I;I=Ḥ stripped).
    B had `Inherkhac` (dropped `Ini-` prefix); C had `Inherkhau` (wrong).
    shared_with_tombs = ["TT359"] (also owner of tomb 359).
    """
    r = _row("TT299")
    ...
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert "Foreman of the Lord of the Two Lands" in r["notes_from_pm"]
    assert "tomb 359" in r["notes_from_pm"]
    assert "Ramesses III" in r["notes_from_pm"]
    assert "Hay" in r["notes_from_pm"]
    assert "Wab" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
...
def test_chunk39_all_rows_present() -> None:
    """All 10 TT301-TT310 rows must be present in reconciled.jsonl."""
    actual = {r["tomb_id"] for r in _rows()} & CHUNK39_TOMB_IDS
    assert actual == CHUNK39_TOMB_IDS, sorted(CHUNK39_TOMB_IDS - actual)
...
def test_tt302_paraemhab() -> None:
    ...
    EGYPTOLOGIST REVIEW REQUIRED: father-name diacritics.
    """
    r = _row("TT302")
...
def test_tt305_paser_waab() -> None:
    ...
    co_occupants.name (strip-ḥ for matchable-name field).
    EGYPTOLOGIST REVIEW REQUIRED: wife-name form.
    """
    r = _row("TT305")
    assert r["occupant_name"] == "Paser"
    assert r["occupant_role"] == "Official"
...
def test_tt306_irzanen() -> None:
    ...
    EGYPTOLOGIST REVIEW REQUIRED: Irzanen vs Irzana.
    """
    r = _row("TT306")
...
def test_tt308_kemsit_deir_el_bahari() -> None:
    ...
    EGYPTOLOGIST REVIEW REQUIRED: all diacritics from PM I.1 p.385 printed source.
    """
    r = _row("TT308")
...
def test_tt309_anonymous_blocked() -> None:
    """TT309 — Name unknown. Sh. ʿAbd el-Qurna. p.386. Blocked.

    occupant_name=None, occupant_role=None. `(Blocked.)` parenthesised per PM.
    """
    r = _row("TT309")
    assert r["occupant_name"] is None
    assert r["occupant_role"] is None
...
def test_tt317_dhutnufer_sh_abd_el_qurna() -> None:
    """TT317 — Ḏhutnufer, Scribe. Sh. ʿAbd el-Qurna. p.390.

    d-bar Ḏ restored by CHUNK40. CHAMPOLLION No. 25 before Parents.
    attribution_certainty=attested via DERIVER_OVERRIDE (regnal-date hedge).
    EGYPTOLOGIST REVIEW REQUIRED: confirm d-bar from printed source.
    """
    r = _row("TT317")
...
def test_tt318_amenmosi_sh_abd_el_qurna() -> None:
    """TT318 — Amenmosi, Necropolis-stonemason. Sh. ʿAbd el-Qurna. p.391.

    CHAMPOLLION No. 26 before Wife per PM source order.
    attribution_certainty=attested via DERIVER_OVERRIDE (regnal-range-tail hedge).
    """
    r = _row("TT318")
    assert r["occupant_name"] == "Amenmosi"
    assert r["occupant_role"] == "Official"
...
def test_tt320_inhapi_royal_cache() -> None:
    ...
    attribution_certainty=attested via DERIVER_OVERRIDE (spousal hedge).
    EGYPTOLOGIST REVIEW REQUIRED: confirm occupant_name from PM I.1 p.392.
    """
    r = _row("TT320")
...
def test_chunk40_all_rows_present() -> None:
    """All 10 TT311-TT320 rows must be present in reconciled.jsonl."""
    actual = {r["tomb_id"] for r in _rows()} & CHUNK40_TOMB_IDS
    assert actual == CHUNK40_TOMB_IDS, sorted(CHUNK40_TOMB_IDS - actual)
...
# ===========================================================================
# Per-row tests — Chunk 41 (TT321–TT330)
# ===========================================================================


def test_tt321_khaemopet_servant() -> None:
    """TT321 — Khaʿemopet, Servant in the Place of Truth. Deir el-Medina. p.393.

    Ramesside. Parents clause restored by CHUNK41 (majority A+B dropped it).
    occupant_name ayin restored to Khaʿemopet (majority B+C).
    EGYPTOLOGIST REVIEW REQUIRED: confirm parent name Busentef (OCR Bu~entef).
    """
    r = _row("TT321")
    assert r["occupant_name"] == "Khaʿemopet"
    assert r["occupant_role"] == "Official"
    ...
    assert r["location_sub_area"] is None
    assert "Ramesside" in r["notes_from_pm"]
    assert "Parents (perhaps)" in r["notes_from_pm"]
    assert "Busentef" in r["notes_from_pm"]
    assert "tomb 219" in r["notes_from_pm"]
    assert "Maani" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 393


def test_tt322_penshenoabu_servant() -> None:
    """TT322 — Penshenʿabu, Servant in the Place of Truth. Deir el-Medina. p.393.

    Ramesside. occupant_name ayin restored (TT322|occupant_name tie-break, pin B).
    source_citation.page majority-wrong 394 → 393 by CHUNK41.
    """
    r = _row("TT322")
    assert r["occupant_name"] == "Penshenʿabu"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["tomb_aliases"] == []
    ...
    assert r["location_sub_area"] is None
    assert "Ramesside" in r["notes_from_pm"]
    assert "Tentnubt" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 393


def test_tt323_peshedu_draughtsman() -> None:
    """TT323 — Peshedu, Outline-draughtsman of Amun. Deir el-Medina. p.394.

    Temp. Sethos I. source_citation.page majority-wrong 395 → 394 by CHUNK41.
    Parent name Amenemḥet restored (OCR `Ameneml_tet` → `Amenemḥet`).
    EGYPTOLOGIST REVIEW REQUIRED: confirm Amenemḥet from PM I.1 p.394 printed source.
    """
    r = _row("TT323")
    assert r["occupant_name"] == "Peshedu"
    assert r["occupant_role"] == "Official"
    ...
    assert "Sethos I" in r["notes_from_pm"]
    assert "Amenemḥet" in r["notes_from_pm"]
    assert "Mutnefert" in r["notes_from_pm"]
    assert "Nefertere" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 394


def test_tt324_hatiay_high_priest() -> None:
    """TT324 — Hatiay, Chief prophet of Sobk. Sh. ʿAbd el-Qurna. p.395.

    Ramesside. occupant_role corrected Official → High Priest by CHUNK41
    (Chief prophet = First prophet = High Priest class; chunk-39 TT303 precedent).
    shared_with_tombs=[TT331] (son Penne's tomb).
    """
    r = _row("TT324")
    assert r["occupant_name"] == "Hatiay"
    assert r["occupant_role"] == "High Priest"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["tomb_aliases"] == []
    assert r["co_occupants"] == []
    assert r["shared_with_tombs"] == ["TT331"]
    assert r["is_joint_burial"] is False
    assert r["is_uninscribed"] is False
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert r["location_sub_area"] is None
    assert "Chief prophet of Sobk" in r["notes_from_pm"]
    assert "Overseer of the prophets" in r["notes_from_pm"]
    assert "Ramesside" in r["notes_from_pm"]
    assert "Nefertere" in r["notes_from_pm"]
    assert "Iuy" in r["notes_from_pm"]
    assert r["theban_area"] == "Sh. ʿAbd el-Qurna"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 395


def test_tt325_anonymous_possibly_smen() -> None:
    """TT325 — Anonymous. Deir el-Medina. p.396. Dyn. XVIII.

    Possibly Smen (from cones). attribution_certainty=uncertain (deriver fires
    on 'Possibly'). occupant_role=Unknown restored by CHUNK41 (sentinel-null
    collapse from all-3-agents 'Unknown' → null at merge; same pattern as
    KV12/TT58/TT70/TT91).
    """
    r = _row("TT325")
    assert r["occupant_name"] is None
    ...
    assert r["is_usurped"] is False
    assert r["location_sub_area"] is None
    assert "Possibly" in r["notes_from_pm"]
    assert "Smen" in r["notes_from_pm"]
    assert "Dyn. XVIII" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 396


def test_tt326_peshedu_foreman_see_tomb3() -> None:
    """TT326 — Peshedu, Foreman. (See tomb 3.) Deir el-Medina. p.396.

    Ramesside. shared_with_tombs=["TT3"] (majority A+C; back-ref symmetry
    with test_chunk9_shared_with_tombs TT3→[TT326]).
    source_citation.page majority-wrong 397 → 396 by CHUNK41.
    """
    r = _row("TT326")
    assert r["occupant_name"] == "Peshedu"
    assert r["occupant_role"] == "Official"
    ...
    assert r["tomb_aliases"] == []
    assert r["co_occupants"] == []
    assert r["shared_with_tombs"] == ["TT3"]
    assert r["is_joint_burial"] is False
    assert r["is_uninscribed"] is False
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert r["location_sub_area"] is None
    assert "Foreman" in r["notes_from_pm"]
    assert "tomb 3" in r["notes_from_pm"]
    assert "Ramesside" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 396
...
def test_tt327_turobay_servant() -> None:
    """TT327 — Turobay, Servant in the Place of Truth. Deir el-Medina. p.397.

    Ramesside. All fields unanimous across agents.
    """
    r = _row("TT327")
    assert r["occupant_name"] == "Turobay"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["tomb_aliases"] == []
    ...
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 397


def test_tt328_hay_servant_dyn20() -> None:
    """TT328 — Hay, Servant in the Place of Truth. Deir el-Medina. p.397. Dyn. XX.

    Wife Tatemeḥet diacritic restored by CHUNK41 (majority A+C dropped underdot-ḥ;
    source line 250 `Tatemel].et` = Tatemeḥet).
    """
    r = _row("TT328")
    assert r["occupant_name"] == "Hay"
    assert r["occupant_role"] == "Official"
    ...
    assert r["location_sub_area"] is None
    assert "Dyn. XX" in r["notes_from_pm"]
    assert "Tatemeḥet" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 397


def test_tt329_mosi_annexed_tomb() -> None:
    """TT329 — Mosi (primary) + annexed tomb of Mosi (grandson) + Ipy. p.397.

    Ramesside. Hierarchical annexed-tomb structure, NOT coordinate co-burial:
    is_joint_burial=False (A+C majority correct; B=true wrong).
    co_occupants=[Mosi(grandson), Ipy] (B+C majority; A malformed missing name).
    OCR names corrected by CHUNK41: Ḥenutwact, Icoḥnufer, Patet.
    attribution_certainty=attested via DERIVER_OVERRIDE (co-occupant kinship hedges).
    EGYPTOLOGIST REVIEW REQUIRED: confirm Ḥenutwact, Icoḥnufer, Patet from PM I.1 p.397.
    """
    r = _row("TT329")
    assert r["occupant_name"] == "Mosi"
    assert r["occupant_role"] == "Official"
    ...
    assert r["occupant_alt_names"] == []
    assert r["tomb_aliases"] == []
    assert len(r["co_occupants"]) == 2
    assert r["co_occupants"][0]["name"] == "Mosi"
    assert r["co_occupants"][1]["name"] == "Ipy"
    assert r["shared_with_tombs"] == []
    assert r["is_joint_burial"] is False
    assert r["is_uninscribed"] is False
    assert r["is_unfinished"] is False
    assert r["is_usurped"] is False
    assert r["location_sub_area"] is None
    assert "Annexed tomb" in r["notes_from_pm"]
    assert "Ḥenutwact" in r["notes_from_pm"]
    assert "Icoḥnufer" in r["notes_from_pm"]
    assert "Patet" in r["notes_from_pm"]
    assert "Louvre, C. 280" in r["notes_from_pm"]
    assert "Bakt" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 397


def test_tt330_karo_servant_dyn19() -> None:
    """TT330 — Karo, Servant in the Place of Truth. Deir el-Medina. p.398. Dyn. XIX.

    Parents Simut and Peshedu. Wife Takhaʿ ayin restored by CHUNK41
    (tie-break pinned C's `Takhac` raw OCR; CHUNK41 restores `<` → ʿ).
    """
    r = _row("TT330")
    assert r["occupant_name"] == "Karo"
    assert r["occupant_role"] == "Official"
    assert r["attribution_certainty"] == "attested"
    assert r["occupant_alt_names"] == []
    assert r["tomb_aliases"] == []
    ...
    assert "Takhaʿ" in r["notes_from_pm"]
    assert r["theban_area"] == "Deir el-Medina"
    assert r["dynasty"] is None
    assert r["source_citation"]["page"] == 398


# ===========================================================================
# Chunk 41 — TT321–TT330 (Deir el-Medina × 9, Sh. ʿAbd el-Qurna × 1)
# ===========================================================================


def test_chunk41_all_rows_present() -> None:
    """All 10 TT321-TT330 rows must be present in reconciled.jsonl."""
    actual = {r["tomb_id"] for r in _rows()} & CHUNK41_TOMB_IDS
    assert actual == CHUNK41_TOMB_IDS, sorted(CHUNK41_TOMB_IDS - actual)


def test_chunk41_theban_areas() -> None:
    """Verify area assignments. TT321-TT323/TT325-TT330 Deir el-Medina; TT324 Sh. Abd el-Qurna."""
    expected = {
        "TT321": "Deir el-Medina",
        "TT322": "Deir el-Medina",
        "TT323": "Deir el-Medina",
        "TT324": "Sh. ʿAbd el-Qurna",
        "TT325": "Deir el-Medina",
        "TT326": "Deir el-Medina",
        "TT327": "Deir el-Medina",
        "TT328": "Deir el-Medina",
        "TT329": "Deir el-Medina",
        "TT330": "Deir el-Medina",
    }
    for tid, area in expected.items():
        r = _row(tid)
        assert r["theban_area"] == area, f"{tid}: expected {area!r}, got {r['theban_area']!r}"
