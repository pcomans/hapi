"""Harvard normalize asset — reads raw Harvard data and writes canonical artifacts."""
...
from pipeline.types.models import artifacts_table, raw_harvard_table
...
def normalize_harvard(context: AssetExecutionContext, database: DatabaseResource) -> None:
    """Map all raw Harvard records to canonical schema and write to artifacts table."""
    mapper = HarvardMapper()
    engine = database.get_engine()

    with engine.connect() as conn:
        result = conn.execute(sa.select(raw_harvard_table))
        raw_rows = result.mappings().all()

    context.log.info(f"Found {len(raw_rows)} raw Harvard records to normalize")
