...
Hölbl 2001 Appendix pp 318–353 covers the Argead bridge (Alexander III → Philip
III Arrhidaios → Alexander IV, 332–310 BCE) in three reign banners. The three
rows are well-constrained — the only fields that meaningfully differ between
agents are the `note` prose cells — but the three-subagent + majority-vote
pipeline is kept for consistency with the other Phase-0 sources and for the
invariant it guarantees on structured fields (holbl_id, display, start_year,
end_year, approximate, page) being reproduced identically by ≥2 of 3 agents.

Structure cloned from kitchen-tipe/merge.py with three adaptations:

1. `kitchen_id` → `holbl_id` everywhere.
2. `STREAM_ORDER` is a single-stream `{"argead": (0, 0)}` — all three rows
   share the `argead` prefix, so the sort key degenerates to sequence order.
3. `DEFAULT_AGENT_DIR` and the environment variable are Hölbl-specific.
...
# Single stream; sort key collapses to sequence order.
...
def _sort_key(hid: str) -> tuple[int, int, str, int]:
    m = _HID_RE.match(hid)
    if not m:
        return (9999, 9999, hid, -1)
