...
- **Chronology dates / `chronologies` dict (pre-Roman rulers only).** Redundant with the project's primary chronology sources (Beckerath 1997, HKW 2006, Kitchen 1996) for everything Beckerath covers (Dyn 0–31, ending at 332 BCE). pharaoh.se's own dict is second-hand to the sources IT cites (Beckerath, Shaw, Dodson, etc.); the project now uses those primaries directly. **Roman emperors (30 rows) retain their chronology from pharaoh.se** — Beckerath stops at 332 BCE and no other Phase-0 source covers the imperial period until Bagnall & Rathbone 2004 lands. Use `sources/beckerath-1997-chronologie/` (lead) + `sources/hkw-chronology-2006/` (fallback) + `sources/kitchen-tipe/` (Dyn 21–26 finer grain) via the plural-named-chronologies map in `rulers.json::chronologies` instead.
