...
- **Roman emperors (30 rows)** — pharaoh.se is currently the ONLY on-disk Phase-0 source covering imperial-period pharaohs (Augustus → Maximinus Daia, c. 30 BCE – c. 313 CE — last hieroglyphic-cartouche emperor in the Egyptian record, at Tahta — with Egyptian throne-name cartouches). This is the cluster preventing outright drop. Replaced post-v0 (issue #166) once a Roman-emperor authority lands; 2026-05-02 source-shape evaluation rejected **Bagnall & Rathbone 2004** as running prose rather than a tabulated regnal-year reference. New candidates: **Kienast *Römische Kaisertabelle*** (regnal-year table) + **Beckerath 1999 *Handbuch*** (Egyptian cartouche names — already in `proprietary/books/`).
...
- **Chronology dates / `chronologies` dict (rulers covered by primary sources only).** Redundant with the project's primary chronology sources (Beckerath 1997, HKW 2006, Kitchen 1996) for everything Beckerath covers (Dyn 0–31, ending 332 BCE). pharaoh.se's own dict is second-hand to the sources IT cites (Beckerath, Shaw, Dodson, etc.); the project now uses those primaries directly. **Argead (3 rows), Ptolemaic (16 rows), Roman emperors (30 rows), and unplaced kings retain their chronology from pharaoh.se** — Beckerath stops at 332 BCE and no other Phase-0 source carries the post-Alexander BCE/CE reign dates until a Roman-period source (Kienast + Beckerath 1999, deferred post-v0 per issue #166; B&R 2004 rejected) and an Argead/Ptolemaic chronology source land. Use `sources/beckerath-1997-chronologie/` (lead) + `sources/hkw-chronology-2006/` (fallback) + `sources/kitchen-tipe/` (Dyn 21–26 finer grain) via the plural-named-chronologies map in `rulers.json::chronologies` instead.
