# Extraction prompt — Porter & Moss Vol I.1 (Theban Necropolis), Chunk 12

> **Fourth chunk drawn from PM I.1 (Private Tombs).** Chunks 9 (TT1–TT10), 10 (TT11–TT20), and 11 (TT21–TT30) established the PM-I.1 conventions including heterogeneous sub-sites, usurpation patterns, cross-valley references, and Vizier-precedence. This prompt is **self-contained** — the agent does NOT need to read prior chunks' prompts; every field rule, schema invariant, and noise pattern is documented here in full. (Per chunk-10 code-reviewer P1 #3: cross-prompt references are fragility risk; each subagent reads exactly one prompt.)
...
- Section: **I. Numbered Tombs**, TT31–TT40 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.65 (printed p.47, capturing the **TT31 headword**) and extends through physical p.96 (printed p.78) so you can see the **TT41 headword** at the tail as a boundary marker — **do NOT extract any TT41+ rows**. Stop at the end of TT40's headword block. The chunk is larger than chunks 9–11 (32 pages vs ~16) because several tombs in this range carry multi-page body-prose blocks between the headword and the next tomb's headword. Body prose is out of scope; filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk12.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk12.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk12.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT31 first, TT40 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
## Expected TT numbering in PM I.1 § I (TT31–TT40 range)

A pre-extraction headword scan of the chunk file confirms PM I.1 catalogues all ten numbers TT31 through TT40 in this section: **TT31, TT32, TT33, TT34, TT35, TT36, TT37, TT38, TT39, TT40**. **10 rows expected** — every number in the decade is present, no gaps in this range.

If your headword scan returns a number outside this range, RE-CHECK the chunk file — it may be (a) a scene-number marker like `(7)` or `(8)` (not a tomb row), (b) a plate-caption tomb-number reference (PM I.1 prints shared floor-plan captions of the form `TOMBS <list-of-numbers>` mid-section), (c) a cross-reference from one tomb's headword to another (`also owner of tomb N` / `(name in tomb N)`), (d) the TT41+ headword near the chunk tail (boundary marker, OUT of scope), or (e) a publisher running-header `Tombs N and N+1` or `Tomb N` page-header. None of those are tomb headwords for THIS chunk.

## PM-I.1 text-layer noise (chunk-12-relevant)

`postprocess.py` runs against this chunk file and normalises the patterns shared with chunks 1–10 (Ḥ-substitutes including `J.I` → `Ḥ`; `<` adjacent to letter → `ʿ`; whitelisted Egyptian-name `c` → `ʿ`; `Ist ed.` → `1st ed.`; king-name-anchored Roman-numeral fixes). The following residual patterns are NOT yet whitelisted in the postprocessor and are handled at prompt + `CHUNK12_CORRECTIONS` level (post-merge in `fix_rows.py`). None of them have unique-bigram signatures stable enough across chunks to safely promote to postprocess (collision risk with non-tomb-headword content is too high — see chunk-10 transcribe.md noise inventory for the precedent).
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT31–TT40). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters.

### `theban_area`

**Per-row extraction.** Like chunk 11, chunk 12 spans MULTIPLE Theban sub-sites. Each TT31–TT40 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row.
...
Honorific prefixes that PM uses to introduce role (an all-caps `KING`, `PRINCE`, `QUEEN`, `VIZIER` before the name) — strip from `occupant_name`, capture in `occupant_role`. Most TT31–TT40 headwords do not lead with such a prefix; the role is given by the title clause AFTER the name (see `occupant_role` rules).

### `occupant_alt_names`

Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-12 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` for every TT31–TT40 row. None of the chunk-12 headwords are expected to carry a sub-site flag like `Eastern Cemetery.` — `theban_area` already captures the sub-site. If a headword in this range declares a finer-grained sub-area inside its sub-site, populate `location_sub_area` and report it.
...
- **Usurpation clauses** (NEW for chunk 12): `Partly usurped by <USURPER-NAME>, <USURPER-ROLE>` — preserve verbatim. The Tier-3 `is_usurped` deriver fires on the `usurp` regex match.
- **Tomb-state-marker parentheticals on the sub-site line** (NEW for chunk 12): a parenthesised state-marker following the sub-site declaration (shapes like `(Inaccessible.)`, `(Unexcavated.)`, `(Reburied.)`) — preserve verbatim. There is no `is_inaccessible` / `is_unexcavated` schema field; the prose stays in `notes_from_pm`.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-12 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Name collisions across decades.** Two distinct individuals can share a name across different TT-decades (chunk-10's TT17 already established this for one common-name slot — extending into chunk 12 may surface another). The schema row-key is `tomb_id`, not name; apply each row's own headword for role / regnal / sub-site fields without conflating.
- **Boundary marker — TT41.** The chunk file extends through physical p.96 to include the start of TT41's headword so you can see the boundary clearly. **Do not extract any TT41 row.** Stop at the end of TT40's headword block.
...
4. **`theban_area` is per-row** (chunk 12 spans 4+ sub-sites including the **NEW `Qurnet Muraʿi`** form). Use the canonical sub-site list above; ASCII apostrophe in `Draʿ Abû el-Nagaʿ`, U+02BF ayin in `ʿAsâsîf` / `Sh. ʿAbd el-Qurna` / `Qurnet Muraʿi`, circumflex preserved in `ʿAsâsîf`. Read each headword's sub-site line and assign per row.
...
13. **Multi-wife pattern variants** (NEW for chunk 12): `Wives, <Name-A> and <Name-B> or <Name-C>` (the `or` indicates `<Name-C>` is an alternate-name reading of `<Name-B>`, NOT a third wife — preserve verbatim including the `or`); `Wives, <Name>, <Name> (name in tomb), and <Name> (names from cones)` (multi-wife with object-cite per-wife parentheticals identifying which wife is attested where — preserve verbatim with all parentheticals). All multi-wife clauses go in `notes_from_pm` per the family-clause rule, NOT in `co_occupants` (wives are not buried in the tomb).
...
- Per-row `theban_area` assignments (chunk 12 is the second heterogeneous chunk after chunk 11; expect rows across the canonical sub-site list above, including the NEW `Qurnet Muraʿi` form).
