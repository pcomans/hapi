...
- Section: **I. Numbered Tombs**, TT281–TT290 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.382 (printed p.364, capturing the **TT281 headword** which opens after TT280 closure on the same page) and extends through physical p.392 (printed p.374) so you can see the **TT291 headword** at the tail as a boundary marker — **do NOT extract any TT291+ rows**. Stop at the end of TT290's headword block. The chunk is 11 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk37.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk37.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk37.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT281 first, TT290 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (281–290)
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT281–TT290). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT281–TT290 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
**Temple-or-non-tomb headwords (THIS CHUNK ONLY).** TT281's headword in PM I.1 reads `281. Unfinished Temple of MENTUḤOTP-SʿANKHKAREʿ. See Bibl. ii, p. 135.` — this is PM's `Unfinished Temple of <KING-PRENOMEN-COMPOUND>` shape: TT281 is NOT a private tomb. PM keeps the tomb-number for the (unfinished) royal mortuary temple of Mentuhotp III. Per chunk-7 KING-MORTUARY-TEMPLE / royal-mortuary precedent (DAN-AhmoseNefertere, etc.), emit: `occupant_name="Mentuhotp-Sʿankhkareʿ"` (strip-Ḥ + ayin restored, the prenomen compound); `occupant_role="King"`; `is_uninscribed=False`; `is_unfinished=True` (PM explicitly states `Unfinished Temple`); `theban_area` from any sub-site declaration in the body; verbatim PM headword text in `notes_from_pm` including `See Bibl. ii, p. 135.` cite. **TT281 is in-scope** for this chunk despite being a temple — it carries a private-tomb number in PM I.1 § I.
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-37 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-37 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT291.** The chunk file extends through physical p.392 to include the start of TT291's headword. **Do not extract any TT291+ rows (TT291 is the first row of the next decade; stop at TT290).** Stop at the end of TT290's headword block. (TT281 IS in scope and IS the first row to extract.)
