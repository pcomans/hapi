...
- **`"Dra' Abu el-Naga"`** — Dra' Abu el-Naga (ASCII apostrophe after `Dra`, no trailing apostrophe — chunk-7 / chunk-10 precedent for the `DAN-` / TT11–TT20 cluster).
...
4. **`theban_area` is per-row** (chunk 11 spans 3+ sub-sites). Use the canonical sub-site list above; ASCII apostrophe in `Dra' Abu el-Naga`, U+02BF ayin in `ʿAsâsîf` and `Sh. ʿAbd el-Qurna`, circumflex preserved in `ʿAsâsîf`. Read each headword's sub-site line and assign per row.
