# Extraction prompt — Porter & Moss Vol I.1 (Theban Necropolis), Chunk 13

> **Fifth chunk drawn from PM I.1 (Private Tombs).** Chunks 9 (TT1–TT10), 10 (TT11–TT20), 11 (TT21–TT30), and 12 (TT31–TT40) established the PM-I.1 conventions including heterogeneous sub-sites, usurpation patterns, cross-valley references, Vizier-precedence, capital-macron restoration, and the d-emphatic d-bar (`Ḏ` U+1E0E) convention for the Thoth/Djehuty name family. This prompt is **self-contained** — the agent does NOT need to read prior chunks' prompts; every field rule, schema invariant, and noise pattern is documented here in full. (Per chunk-10 code-reviewer P1 #3: cross-prompt references are fragility risk; each subagent reads exactly one prompt.)

You are one of three independent extraction subagents. Read the text-layer chunk file at `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/chunk-p96-p115.txt` and produce a JSONL file with one structured row per numbered tomb in PM I.1 § I "Numbered Tombs" within the **TT41–TT50** range. The other two agents see the same prompt and the same chunk; their outputs are majority-voted by `merge.py`.

**Prompt discipline (per CLAUDE.md rules 1 and 7):** this prompt gives field-extraction RULES and normalisation conventions — it does NOT hand you per-tomb answers. Every field value must trace to something in the chunk file. The only things the prompt may validly hint: structural facts about the section (page range, tomb-id range), text-layer noise signatures, vocabulary constraints, and explicit examples drawn from PRIOR landed chunks (chunks 1–12) for analogy. Patterns that appear in chunk 13 (continued heterogeneous sub-sites, the d-emphatic d-bar `Ḏ` consonant in Thoth-derived name family, ongoing capital-macron drop noise) are documented here as RULES, not as per-row values to copy.
...
- Section: **I. Numbered Tombs**, TT41–TT50 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.96 (printed p.78, capturing the **TT41 headword**) and extends through physical p.115 (printed p.97) so you can see the **TT51 headword** at the tail as a boundary marker — **do NOT extract any TT51+ rows**. Stop at the end of TT50's headword block. The chunk is 20 physical pages because several tombs in this range carry multi-page body-prose blocks between the headword and the next tomb's headword. Body prose is out of scope; filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk13.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk13.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk13.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT41 first, TT50 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (41–50)
...
## Expected TT numbering in PM I.1 § I (TT41–TT50 range)

A pre-extraction headword scan of the chunk file confirms PM I.1 catalogues all ten numbers TT41 through TT50 in this section: **TT41, TT42, TT43, TT44, TT45, TT46, TT47, TT48, TT49, TT50**. **10 rows expected** — every number in the decade is present, no gaps in this range.
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT41–TT50). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Like chunks 11–12, chunk 13 spans multiple Theban sub-sites. Each TT41–TT50 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row.
...
Honorific prefixes that PM uses to introduce role (an all-caps `KING`, `PRINCE`, `QUEEN`, `VIZIER` before the name) — strip from `occupant_name`, capture in `occupant_role`. Most TT41–TT50 headwords do not lead with such a prefix; the role is given by the title clause AFTER the name (see `occupant_role` rules).

### `occupant_alt_names`

Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-13 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` for every TT41–TT50 row. None of the chunk-13 headwords are expected to carry a sub-site flag like `Eastern Cemetery.` — `theban_area` already captures the sub-site. If a headword in this range declares a finer-grained sub-area inside its sub-site, populate `location_sub_area` and report it.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-13 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT51.** The chunk file extends through physical p.115 to include the start of TT51's headword so you can see the boundary clearly. **Do not extract any TT51 row.** Stop at the end of TT50's headword block.
...
1. **10 rows expected** (every TT number in TT41..TT50 has a headword in PM I.1 § I — no gaps in this decade).
...
4. **`theban_area` is per-row** (chunk 13 continues the heterogeneous pattern from chunks 11–12). Use the canonical sub-site list above; ASCII apostrophe in `Draʿ Abû el-Nagaʿ`, U+02BF ayin in `ʿAsâsîf` / `Sh. ʿAbd el-Qurna` / `Qurnet Muraʿi`, circumflex preserved in `ʿAsâsîf`. Read each headword's sub-site line and assign per row.
...
- Per-row `theban_area` assignments (chunk 13 continues the heterogeneous pattern from chunks 11–12; expect rows across the canonical sub-site list above).
