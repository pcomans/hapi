...
4. **`theban_area` is per-row** (chunk 13 continues the heterogeneous pattern from chunks 11–12). Use the canonical sub-site list above; U+02BF ʿayin in `Draʿ Abû el-Nagaʿ`, U+02BF ayin in `ʿAsâsîf` / `Sh. ʿAbd el-Qurna` / `Qurnet Muraʿi`, circumflex preserved in `ʿAsâsîf`. Read each headword's sub-site line and assign per row.
