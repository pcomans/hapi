...
- Section: **I. Numbered Tombs**, TT361–TT370 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.444 (printed p.426, capturing the **TT361 headword** which opens after TT360 closure on the same page) and extends through physical p.450 (printed p.432) so you can see the **TT371 headword** at the tail as a boundary marker — **do NOT extract any TT371+ rows**. Stop at the end of TT370's headword block. The chunk is 7 physical pages (a smaller page span than the usual 10 because the average entry length in this decade is shorter than the prior decades); body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk45.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk45.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk45.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT361 first, TT370 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (361–370)
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT361–TT370). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT361–TT370 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
**Additional anonymous-form variants** (extend the catalog as PM phrasings surface chunk-to-chunk; the worked examples below are placeholder shapes, not chunk-45 row content):
...
- A headword whose name token is reduced to a short surviving fragment preceded by an ellipsis-dot run: `<NUMBER>. . . . <SURVIVING-FRAGMENT> <cartouche-garbage>, <title>` (PM prints the partly-lost name as a sequence of spaced ellipsis dots followed by whatever fragment of the name survives). Per the PM-faithful policy this is a damaged-text headword analogous to the bracketed-fragment case (see below). Emit `occupant_name` as the verbatim partial-name string preserving PM's exact typography (three ASCII periods, one ASCII space between each dot, one ASCII space before the surviving fragment, then the fragment itself with the strip-Ḥ / preserve-Ḳ / preserve-Ḏ / preserve-ayin / strip-macron rules applied to the fragment). Do NOT recast the dots as `[…]` brackets, do NOT collapse the spaced dots into `...` or `…`, do NOT emit `null` (the surviving fragment carries information that supports cross-source matching). The `occupant_role` follows the controlled-vocab rules from the title (NOT `"Unknown"` — the role is fully attested even when only a fragment of the name survives).
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-45 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-45 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT371.** The chunk file extends through physical p.450 to include the start of TT371's headword. **Do not extract any TT371+ rows (TT371 is the first row of the next decade; stop at TT370).** Stop at the end of TT370's headword block. (TT361 IS in scope and IS the first row to extract.)
