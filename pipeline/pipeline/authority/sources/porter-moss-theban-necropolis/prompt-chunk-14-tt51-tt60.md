# Extraction prompt — Porter & Moss Vol I.1 (Theban Necropolis), Chunk 14

> **Sixth chunk drawn from PM I.1 (Private Tombs).** Chunks 9 (TT1–TT10), 10 (TT11–TT20), 11 (TT21–TT30), 12 (TT31–TT40), and 13 (TT41–TT50) established the PM-I.1 conventions including heterogeneous sub-sites, usurpation patterns, cross-valley references, Vizier-precedence, capital-macron restoration, and the d-emphatic d-bar (`Ḏ` U+1E0E) convention for the Thoth/Djehuty name family. This prompt is **self-contained** — the agent does NOT need to read prior chunks' prompts; every field rule, schema invariant, and noise pattern is documented here in full. (Per chunk-10 code-reviewer P1 #3: cross-prompt references are fragility risk; each subagent reads exactly one prompt.)

You are one of three independent extraction subagents. Read the text-layer chunk file at `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/chunk-p115-p141.txt` and produce a JSONL file with one structured row per numbered tomb in PM I.1 § I "Numbered Tombs" within the **TT51–TT60** range. The other two agents see the same prompt and the same chunk; their outputs are majority-voted by `merge.py`.

**Prompt discipline (per CLAUDE.md rules 1 and 7):** this prompt gives field-extraction RULES and normalisation conventions — it does NOT hand you per-tomb answers. Every field value must trace to something in the chunk file. The only things the prompt may validly hint: structural facts about the section (page range, tomb-id range), text-layer noise signatures, vocabulary constraints, and explicit examples drawn from PRIOR landed chunks (chunks 1–13) for analogy. Patterns that appear in chunk 14 (continued heterogeneous sub-sites, the d-emphatic d-bar `Ḏ` consonant in Thoth-derived name family, ongoing capital-macron drop noise, anonymous-occupant headwords, Vizier-role headwords) are documented here as RULES, not as per-row values to copy.
...
- Section: **I. Numbered Tombs**, TT51–TT60 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.115 (printed p.97, capturing the **TT51 headword**) and extends through physical p.141 (printed p.123) so you can see the **TT61 headword** at the tail as a boundary marker — **do NOT extract any TT61+ rows**. Stop at the end of TT60's headword block. The chunk is 27 physical pages because several tombs in this range (TT55, TT57, TT60) carry multi-page body-prose blocks between the headword and the next tomb. Body prose is out of scope; filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk14.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk14.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk14.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT51 first, TT60 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (51–60)
...
## Expected TT numbering in PM I.1 § I (TT51–TT60 range)

A pre-extraction headword scan of the chunk file confirms PM I.1 catalogues all ten numbers TT51 through TT60 in this section: **TT51, TT52, TT53, TT54, TT55, TT56, TT57, TT58, TT59, TT60**. **10 rows expected** — every number in the decade is present, no gaps in this range.

If your headword scan returns a number outside this range, RE-CHECK the chunk file — it may be (a) a scene-number marker like `(7)` or `(8)` (not a tomb row), (b) a plate-caption tomb-number reference (PM I.1 prints shared floor-plan captions of the form `TOMBS <list-of-numbers>` mid-section), (c) a cross-reference from one tomb's headword to another (`also owner of tomb N` / `(name in tomb N)`), (d) the TT51+ headword near the chunk tail (boundary marker, OUT of scope), or (e) a publisher running-header `Tombs N and N+1` or `Tomb N` page-header. None of those are tomb headwords for THIS chunk.

## PM-I.1 text-layer noise (chunk-14-relevant)

`postprocess.py` runs against this chunk file and normalises the patterns shared with chunks 1–12 (Ḥ-substitutes including `J.I` → `Ḥ`; `<` adjacent to letter → `ʿ`; whitelisted Egyptian-name `c` → `ʿ`; `Ist ed.` → `1st ed.`; king-name-anchored Roman-numeral fixes). The following residual patterns are NOT yet whitelisted in the postprocessor and are handled at prompt + `CHUNK14_CORRECTIONS` level (post-merge in `fix_rows.py`). None of them have unique-bigram signatures stable enough across chunks to safely promote to postprocess (collision risk with non-tomb-headword content is too high — see chunk-10 transcribe.md noise inventory for the precedent).

- **Tomb-number-position digit/letter substitution.** Headword tomb-numbers can render as non-canonical glyph clusters; prior chunks observed `12•` (bullet for period), `zo.` (z for 2 / o for 0), `33·` (middle-dot), `3 I.`-shaped clusters, and colon-for-period (`<NUMBER>:`). Disambiguate by column-zero position followed by an all-caps NAME token; collapse the noise to the canonical `<NUMBER>.` form for `tomb_id`.
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT51–TT60). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT51–TT60 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
- **`"Draʿ Abû el-Nagaʿ"`** — Draʿ Abû el-Nagaʿ (ASCII apostrophe after `Dra`, no trailing apostrophe — chunk-7 / chunk-10 / chunk-11 precedent).
...
Honorific prefixes that PM uses to introduce role (an all-caps `KING`, `PRINCE`, `QUEEN`, `VIZIER` before the name) — strip from `occupant_name`, capture in `occupant_role`. Most TT51–TT60 headwords do not lead with such a prefix; the role is given by the title clause AFTER the name (see `occupant_role` rules).

**Null-name (anonymous-occupant) headwords.** When PM's headword does NOT name the original occupant — typical phrasings (per the chunk-8 KV12/KV39/KV56 + QV36/QV40/QV73/QV75 precedent) include `A QUEEN, no name`, `A PRINCESS, no name, Dyn. XX`, and `<NUMBER>. <NO-NAME-PHRASE>, <subsequent clause>` shapes where no proper-name NAME-IN-CAPS token appears between the tomb-number separator and the title clause — emit `occupant_name=null`. The role flattens to `"Unknown"` per the role rules below; the verbatim phrasing PM uses for the no-name marker goes in `notes_from_pm`. The usurpation rule (above) is structurally independent and applies separately whenever PM uses the `Usurped by` clause regardless of whether the primary occupant is named or anonymous.

### `occupant_alt_names`

Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-14 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` for every TT51–TT60 row. None of the chunk-14 headwords are expected to carry a sub-site flag like `Eastern Cemetery.` — `theban_area` already captures the sub-site. If a headword in this range declares a finer-grained sub-area inside its sub-site, populate `location_sub_area` and report it.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-14 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT61.** The chunk file extends through physical p.141 to include the start of TT61's headword so you can see the boundary clearly. **Do not extract any TT61 row.** Stop at the end of TT60's headword block.
...
1. **10 rows expected** (every TT number in TT51..TT60 has a headword in PM I.1 § I — no gaps in this decade).
