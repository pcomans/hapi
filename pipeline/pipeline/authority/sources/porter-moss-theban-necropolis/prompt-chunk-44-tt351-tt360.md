...
- Section: **I. Numbered Tombs**, TT351–TT360 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.435 (printed p.417, capturing the **TT351 headword** which opens after TT350 closure on the same page) and extends through physical p.444 (printed p.426) so you can see the **TT361 headword** at the tail as a boundary marker — **do NOT extract any TT361+ rows**. Stop at the end of TT360's headword block. The chunk is 10 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk44.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk44.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk44.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT351 first, TT360 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (351–360)
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT351–TT360). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT351–TT360 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
**Additional anonymous-form variants** (extend the catalog as PM phrasings surface chunk-to-chunk; the worked examples below are placeholder shapes, not chunk-44 row content):
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-44 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT361.** The chunk file extends through physical p.444 to include the start of TT361's headword. **Do not extract any TT361+ rows (TT361 is the first row of the next decade; stop at TT360).** Stop at the end of TT360's headword block. (TT351 IS in scope and IS the first row to extract.)
