...
- Section: **I. Numbered Tombs**, TT191–TT200 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.315 (printed p.297, capturing the **TT191 headword** which opens after TT190 closure on the same page) and extends through physical p.323 (printed p.305) so you can see the **TT201 headword** at the tail as a boundary marker — **do NOT extract any TT201+ rows**. Stop at the end of TT200's headword block. The chunk is 9 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk28.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk28.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk28.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT191 first, TT200 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (191–200)
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT191–TT200). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT191–TT200 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-28 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-28 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT201.** The chunk file extends through physical p.323 to include the start of TT201's headword. **Do not extract any TT201+ rows (TT201 is the first row of the next decade; stop at TT200).** Stop at the end of TT200's headword block. (TT191 IS in scope and IS the first row to extract.)
