...
- Section: **I. Numbered Tombs**, TT101–TT110 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.232 (printed p.214, capturing the **TT101 headword** which opens after TT100's content tail on the same page) and extends through physical p.246 (printed p.228) so you can see the **TT111 headword** at the tail as a boundary marker — **do NOT extract any TT111+ rows**. Stop at the end of TT110's headword block. The chunk is 15 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk19.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk19.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk19.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT101 first, TT110 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (101–110)
...
## Expected TT numbering in PM I.1 § I (TT101–TT110 range)

A pre-extraction headword scan suggests PM I.1 catalogues the TT101–TT110 decade. Expected row count is approximately 10, but verify by reading the chunk: PM I.1 has occasional gaps (TT-numbers without entries) and occasional collapsed entries (multiple tomb numbers sharing a single headword block — chunk-19 raw shows page-header pattern "TOMBS 106, 108-13" suggesting a multi-tomb headword in the middle of the decade).

If your headword scan returns a number outside this range, RE-CHECK the chunk file — it may be (a) a scene-number marker like `(7)` or `(8)` (not a tomb row), (b) a plate-caption tomb-number reference (PM I.1 prints shared floor-plan captions of the form `TOMBS <list-of-numbers>` mid-section), (c) a cross-reference from one tomb's headword to another (`also owner of tomb N` / `(name in tomb N)`), (d) the TT101+ headword near the chunk tail (boundary marker, OUT of scope), or (e) a publisher running-header `Tombs N and N+1` or `Tomb N` page-header. None of those are tomb headwords for THIS chunk.

## PM-I.1 text-layer noise (chunk-19-relevant)

`postprocess.py` runs against this chunk file and normalises the patterns shared with chunks 1–12 (Ḥ-substitutes including `J.I` → `Ḥ`; `<` adjacent to letter → `ʿ`; whitelisted Egyptian-name `c` → `ʿ`; `Ist ed.` → `1st ed.`; king-name-anchored Roman-numeral fixes). The following residual patterns are NOT yet whitelisted in the postprocessor and are handled at prompt + `CHUNK19_CORRECTIONS` level (post-merge in `fix_rows.py`). None of them have unique-bigram signatures stable enough across chunks to safely promote to postprocess (collision risk with non-tomb-headword content is too high — see chunk-10 transcribe.md noise inventory for the precedent).
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT101–TT110). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT101–TT110 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
Honorific prefixes that PM uses to introduce role (an all-caps `KING`, `PRINCE`, `QUEEN`, `VIZIER` before the name) — strip from `occupant_name`, capture in `occupant_role`. Most TT101–TT110 headwords do not lead with such a prefix; the role is given by the title clause AFTER the name (see `occupant_role` rules).
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-19 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-19 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT111.** The chunk file extends through physical p.246 to include the start of TT111's headword so you can see the boundary clearly. **Do not extract any TT111+ rows.** Stop at the end of TT110's headword block. (TT101 IS in scope and IS the first row to extract.)
...
1. **10 rows expected** (every TT number in TT101..TT110 has a headword in PM I.1 § I — no gaps in this decade).
