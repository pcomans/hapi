...
- Section: **I. Numbered Tombs**, TT401–TT409 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.462 (printed p.444, capturing the **TT401 headword** which opens after TT400 closure on the same page) and extends through physical p.466 (printed p.448) — there is no TT410 boundary marker since PM I.1's TT-numbering ends at TT409 in this volume. Stop at the end of the LAST numbered-tomb headword block within the TT401–TT409 range. The chunk is 5 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk49.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk49.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk49.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT401 first, TT409 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (401–409)
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (up to 9 expected values: TT401, TT402, TT403, TT404, TT405, TT406, TT407, TT408, TT409 — verify each is present in the chunk). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT401–TT409 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
- **`"Draʿ Abû el-Nagaʿ"`** — Draʿ Abû el-Nagaʿ (ASCII apostrophe after `Dra`, no trailing apostrophe — chunk-7 / chunk-10 / chunk-11 precedent). Note: PM prints `Dra' Abû el-Naga'` with circumflex `û` and terminal ʿayin; the project canonical strips both for matchability across chunks. Cross-chunk migration tracked in issue #288 — do NOT migrate per-chunk.
- **`"ʿAsâsîf"`** — ʿAsâsîf (leading ayin U+02BF; circumflex `â` and `î` preserved per PM's printed typography).
- **`"Khôkha"`** — Khôkha (no diacritics; PM prints in plain Roman).
...
- **`"Deir el-Baḥri"`** — Deir el-Baḥri (no diacritics; PM prints `Deir el-Baḥri` with Ḥ-underdot, but the project canonical strips Ḥ to plain h for matchability — established across chunks 30+ in 30+ rows; do NOT use `Deir el-Baḥri` here. Cross-chunk migration to PM-faithful form tracked in issue #291.) Added to canonical list per PR #290 chunk-44 code-reviewer P3 + egyptologist P2 follow-up.
...
**Additional anonymous-form variants** (extend the catalog as PM phrasings surface chunk-to-chunk; the worked examples below are placeholder shapes, not chunk-49 row content):
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-49 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-49 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **End-of-section boundary.** This is the FINAL chunk of PM I.1 § I "Numbered Tombs" — TT-numbering ends within the TT401–TT409 range and there is no TT410+ in this volume. Stop at the end of the LAST numbered-tomb headword in the chunk. (TT401 IS in scope and IS the first row to extract.) The chunk file may include the start of PM I.1 § II "Tombs without official numbers" or other post-§I content; do NOT extract any of those — only TT-prefixed numbered tombs in the TT401–TT409 range.
...
1. **up to 9 rows expected** (TT401 through TT409, the FINAL § I decade; no TT410+ in this volume) — verify by reading the chunk; PM I.1 has occasional gaps in other decades. If you find fewer than 9 headwords, re-check for anonymous/untitled entries before concluding a gap exists.
...
4. **`theban_area` is per-row.** Use the canonical sub-site list above; ASCII apostrophe in `Draʿ Abû el-Nagaʿ`, U+02BF ayin in `ʿAsâsîf` / `Sh. ʿAbd el-Qurna` / `Qurnet Muraʿi`, circumflex preserved in `ʿAsâsîf`. Read each headword's sub-site line and assign per row.
...
- Row count (up to 9 expected: TT401–TT409, final § I decade) and the complete list of TT tomb_ids you emitted in sort order.
