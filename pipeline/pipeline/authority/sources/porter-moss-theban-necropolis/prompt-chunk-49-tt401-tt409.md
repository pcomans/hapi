...
- **`"Dra' Abu el-Naga"`** — Dra' Abu el-Naga (ASCII apostrophe after `Dra`, no trailing apostrophe — chunk-7 / chunk-10 / chunk-11 precedent). Note: PM prints `Dra' Abû el-Naga'` with circumflex `û` and terminal ʿayin; the project canonical strips both for matchability across chunks. Cross-chunk migration tracked in issue #288 — do NOT migrate per-chunk.
