...
- Section: **I. Numbered Tombs**, TT371–TT380 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.450 (printed p.432, capturing the **TT371 headword** which opens after TT370 closure on the same page) and extends through physical p.453 (printed p.435) so you can see the **TT381 headword** at the tail as a boundary marker — **do NOT extract any TT381+ rows**. Stop at the end of TT380's headword block. The chunk is 4 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk46.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk46.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk46.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT371 first, TT380 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (371–380)
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT371–TT380). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT371–TT380 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
**Additional anonymous-form variants** (extend the catalog as PM phrasings surface chunk-to-chunk; the worked examples below are placeholder shapes, not chunk-46 row content):
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-46 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-46 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT381.** The chunk file extends through physical p.453 to include the start of TT381's headword. **Do not extract any TT381+ rows (TT381 is the first row of the next decade; stop at TT380).** Stop at the end of TT380's headword block. (TT371 IS in scope and IS the first row to extract.)
