...
- Section: **I. Numbered Tombs**, TT221–TT230 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.341 (printed p.323, capturing the **TT221 headword** which opens after TT220 closure on the same page) and extends through physical p.346 (printed p.328) so you can see the **TT231 headword** at the tail as a boundary marker — **do NOT extract any TT231+ rows**. Stop at the end of TT230's headword block. The chunk is 6 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk31.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk31.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk31.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT221 first, TT230 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (221–230)
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT221–TT230). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT221–TT230 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-31 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-31 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT231.** The chunk file extends through physical p.346 to include the start of TT231's headword. **Do not extract any TT231+ rows (TT231 is the first row of the next decade; stop at TT230).** Stop at the end of TT230's headword block. (TT221 IS in scope and IS the first row to extract.)
