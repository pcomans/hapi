...
- Section: **I. Numbered Tombs**, TT311–TT320 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.404 (printed p.386, capturing the **TT311 headword** which opens after TT310 closure on the same page) and extends through physical p.411 (printed p.393) so you can see the **TT321 headword** at the tail as a boundary marker — **do NOT extract any TT321+ rows**. Stop at the end of TT320's headword block. The chunk is 8 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk40.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk40.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk40.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT311 first, TT320 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (311–320)
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT311–TT320). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT311–TT320 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-40 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-40 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT321.** The chunk file extends through physical p.411 to include the start of TT321's headword. **Do not extract any TT321+ rows (TT321 is the first row of the next decade; stop at TT320).** Stop at the end of TT320's headword block. (TT311 IS in scope and IS the first row to extract.)
