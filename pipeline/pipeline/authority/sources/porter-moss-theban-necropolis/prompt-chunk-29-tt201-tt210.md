...
- Section: **I. Numbered Tombs**, TT201–TT210 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.322 (printed p.304, capturing the **TT201 headword** which opens after TT200 closure on the same page) and extends through physical p.327 (printed p.309) so you can see the **TT211 headword** at the tail as a boundary marker — **do NOT extract any TT211+ rows**. Stop at the end of TT210's headword block. The chunk is 6 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk29.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk29.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk29.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT201 first, TT210 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (201–210)
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT201–TT210). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT201–TT210 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-29 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-29 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT211.** The chunk file extends through physical p.327 to include the start of TT211's headword. **Do not extract any TT211+ rows (TT211 is the first row of the next decade; stop at TT210).** Stop at the end of TT210's headword block. (TT201 IS in scope and IS the first row to extract.)
