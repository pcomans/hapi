...
- Section: **I. Numbered Tombs**, TT161–TT170 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.292 (printed p.274, capturing the **TT161 headword** which opens after TT160 closure on the same page) and extends through physical p.297 (printed p.279) so you can see the **TT171 headword** at the tail as a boundary marker — **do NOT extract any TT171+ rows**. Stop at the end of TT170's headword block. The chunk is 6 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk25.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk25.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk25.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT161 first, TT170 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (161–170)
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT161–TT170). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT161–TT170 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-25 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-25 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT171.** The chunk file extends through physical p.297 to include the start of TT171's headword so you can see the boundary clearly. **Do not extract any TT171+ rows.** Stop at the end of TT170's headword block. (TT161 IS in scope and IS the first row to extract.)
