...
- Section: **I. Numbered Tombs**, TT91–TT100 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.203 (printed p.185, capturing the **TT91 headword**) and extends through physical p.233 (printed p.215) so you can see the **TT101 headword** at the tail as a boundary marker — **do NOT extract any TT101+ rows**. Stop at the end of TT100's headword block. The chunk is 31 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk18.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk18.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk18.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT91 first, TT100 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (91–100)
...
## Expected TT numbering in PM I.1 § I (TT91–TT100 range)

A pre-extraction headword scan of the chunk file confirms PM I.1 catalogues all ten numbers TT91 through TT100 in this section: **TT91, TT92, TT93, TT94, TT95, TT96, TT97, TT98, TT99, TT100**. **10 rows expected** — every number in the decade is present, no gaps in this range.
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT91–TT100). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT91–TT100 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
Honorific prefixes that PM uses to introduce role (an all-caps `KING`, `PRINCE`, `QUEEN`, `VIZIER` before the name) — strip from `occupant_name`, capture in `occupant_role`. Most TT91–TT100 headwords do not lead with such a prefix; the role is given by the title clause AFTER the name (see `occupant_role` rules).
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-18 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-18 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT101.** The chunk file extends through physical p.233 to include the start of TT101's headword so you can see the boundary clearly. **Do not extract any TT101+ rows.** Stop at the end of TT100's headword block. (TT91 IS in scope and IS the first row to extract.)
...
1. **10 rows expected** (every TT number in TT91..TT100 has a headword in PM I.1 § I — no gaps in this decade).
