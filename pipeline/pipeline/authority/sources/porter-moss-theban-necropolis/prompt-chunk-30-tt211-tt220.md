...
- Section: **I. Numbered Tombs**, TT211–TT220 range.
- PM I.1 offset: **printed = physical − 18** (distinct from PM I.2's +458 used in chunks 1–8).
- The chunk file starts at physical p.325 (printed p.307, capturing the **TT211 headword** which falls at the bottom of the page after TT210 closure) and extends through physical p.341 (printed p.323) so you can see the **TT221 headword** at the tail as a boundary marker — **do NOT extract any TT221+ rows**. Stop at the end of TT220's headword block. The chunk is 17 physical pages; body prose is out of scope regardless of length — filter on column-zero headword shape.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk30.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk30.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk30.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT211 first, TT220 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (211–220)
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT211–TT220). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT211–TT220 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-30 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-30 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT221.** The chunk file extends through physical p.341 to include the start of TT221's headword. **Do not extract any TT221+ rows (TT221 is the first row of the next decade; stop at TT220).** Stop at the end of TT220's headword block. (TT211 IS in scope and IS the first row to extract.)
