...
- `theban_area` — Coarse Theban sub-area / valley / cemetery name: `"Valley of the Kings"`, `"Valley of the Queens"`, `"Dra' Abu el-Naga"`, `"Deir el-Bahri"`, `"Asasif"`, `"Sheikh Abd el-Qurna"`, `"Khokha"`, `"Qurnet Mura'i"`, `"Deir el-Medina"`, `"Ramesseum"`, `"Medinet Habu"`. The Theban area a tomb belongs to is structural in PM (each numbered tomb sits within a section / sub-section); for PM I.1 numbered tombs the canonical classification is the Appendix-D Theban sub-site list (the field was renamed from `valley` → `theban_area` because PM I.1 sub-sites like Deir el-Medina are not literally valleys).
