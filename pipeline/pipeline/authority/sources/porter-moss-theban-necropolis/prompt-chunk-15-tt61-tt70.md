# Extraction prompt — Porter & Moss Vol I.1 (Theban Necropolis), Chunk 15

> **Seventh chunk drawn from PM I.1 (Private Tombs).** Chunks 9 (TT1–TT10), 10 (TT11–TT20), 11 (TT21–TT30), 12 (TT31–TT40), 13 (TT41–TT50), and 14 (TT51–TT60) established the PM-I.1 conventions including heterogeneous sub-sites, usurpation patterns, cross-valley references, Vizier-precedence, capital-macron restoration, the d-emphatic d-bar (`Ḏ` U+1E0E) convention for the Thoth/Djehuty name family, anonymous-occupant headwords, and hierarchical mother/son co-occupancy. This prompt is **self-contained** — the agent does NOT need to read prior chunks' prompts; every field rule, schema invariant, and noise pattern is documented here in full. (Per chunk-10 code-reviewer P1 #3: cross-prompt references are fragility risk; each subagent reads exactly one prompt.)

You are one of three independent extraction subagents. Read the text-layer chunk file at `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/chunk-p141-p160.txt` and produce a JSONL file with one structured row per numbered tomb in PM I.1 § I "Numbered Tombs" within the **TT61–TT70** range. The other two agents see the same prompt and the same chunk; their outputs are majority-voted by `merge.py`.

**Prompt discipline (per CLAUDE.md rules 1 and 7):** this prompt gives field-extraction RULES and normalisation conventions — it does NOT hand you per-tomb answers. Every field value must trace to something in the chunk file. The only things the prompt may validly hint: structural facts about the section (page range, tomb-id range), text-layer noise signatures, vocabulary constraints, and explicit examples drawn from PRIOR landed chunks (chunks 1–14) for analogy. Patterns that appear in chunk 15 (continued heterogeneous sub-sites, the d-emphatic d-bar `Ḏ` consonant in Thoth-derived name family, ongoing capital-macron drop noise, anonymous-occupant headwords, Vizier-role headwords, bracketed-name-fragment headword `[X]NAME-TAIL` for partially-damaged readings PM marks with editorial brackets) are documented here as RULES, not as per-row values to copy.
...
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-a-chunk15.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-b-chunk15.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-theban-necropolis/raw/agent-c-chunk15.jsonl`

One JSON object per line. Sort rows by numeric `tomb_id` ascending (TT61 first, TT70 last). Use `json.dumps(..., sort_keys=True, ensure_ascii=False)` for deterministic output.
...
```json
{
  "tomb_id": "TT<N>",  // <N> is the printed tomb number (61–70)
...
## Expected TT numbering in PM I.1 § I (TT61–TT70 range)

A pre-extraction headword scan of the chunk file confirms PM I.1 catalogues all ten numbers TT61 through TT70 in this section: **TT61, TT62, TT63, TT64, TT65, TT66, TT67, TT68, TT69, TT70**. **10 rows expected** — every number in the decade is present, no gaps in this range.
...
`TT<N>` where `<N>` is the Arabic tomb number from the heading line (10 expected values: TT61–TT70). Apply the tomb-number-position-noise rule above when the OCR has substituted digits/letters. Some tomb-numbers in this range render with a middle-dot separator (`<N>·` instead of `<N>.`); collapse to canonical `<N>.` for `tomb_id`.

### `theban_area`

**Per-row extraction.** Each TT61–TT70 headword's sub-site line — the line immediately below the title clause — declares which sub-site that row belongs to. Read the chunk and assign per row using the canonical forms below.
...
Honorific prefixes that PM uses to introduce role (an all-caps `KING`, `PRINCE`, `QUEEN`, `VIZIER` before the name) — strip from `occupant_name`, capture in `occupant_role`. Most TT61–TT70 headwords do not lead with such a prefix; the role is given by the title clause AFTER the name (see `occupant_role` rules).
...
Alternate name forms PM gives for the SAME PERSON in the headword block. PM's `<NAME-A>, also called <NAME-B>` phrasing is the canonical pattern (e.g. chunk-7 DAN- prefixed alt-name surfaces). When PM prints this in a chunk-15 headword, capture the secondary name token in `occupant_alt_names`. Apply the same diacritic/noise rules as `occupant_name`. Empty list `[]` when absent.
...
`null` when the headword has nothing beyond the bare name + cartouche garbage + sub-site (rare for chunk-15 — PM I.1 headwords are dense and most rows carry substantial structured content).
...
- **Boundary marker — TT71.** The chunk file extends through physical p.160 to include the start of TT71's headword so you can see the boundary clearly. **Do not extract any TT71+ rows.** Stop at the end of TT70's headword block. (TT61 IS in scope and IS the first row to extract.)
...
1. **10 rows expected** (every TT number in TT61..TT70 has a headword in PM I.1 § I — no gaps in this decade).
