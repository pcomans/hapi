...
## Issue #179 schema additions (2026-05-03 audit-fix, PR pending)

The audit-fix replaces scalar titulary fields with a typed list and adds typed flags. Every row carries every field after `fix_rows.py` runs (closure-tested by `test_179_every_row_has_typed_flags`):

- **`egyptian_titularies: list[{name, kind, when}]`** — canonical replacement for the legacy scalar `egyptian_titulary` + `egyptian_titulary_kind` pair. `kind` ∈ {`nomen`, `prenomen`, `horus_name`, `nebty_name`, `golden_horus_name`} (the legacy `mixed` enum is gone — multi-name rows decompose into multiple list entries with the correct per-entry kind). `when` is null except for 19.07 Si-ptah (`anfangs Secha-en-rê mer-amun, später Ach-en-rê sotep-en-rê` decomposes to two prenomen entries with `when="anfangs"` and `when="später"`). Slash-alternatives split into separate entries (15.05 Apophis: `A-qen-en-rê/A-user-rê` → 2 prenomen entries). Closure-tested by `test_179_egyptian_titularies_kind_in_vocab` and `test_179_egyptian_titularies_shape`.
- **`name_variants: list[str]`** — extracted from `(...)` parens in `name`. For names containing parenthetical content (parens *embedded* within the name), the canonical `name` has the parens stripped and the inner content goes into `name_variants` (12.01 `"Ammenemes (Amen-em-hat) I."` → `name="Ammenemes I."` + `name_variants=["Amen-em-hat"]`; 19.01 multi-variant `"Ramses (Ra-mes-su, griech. Ramessês) I."` → `name_variants=["Ra-mes-su", "griech. Ramessês"]`). Names where the **entire** content is wrapped in parens (e.g. 22.07 `"(Schoschenq IIIa.)"`) are uncertainty markers — the `name` field stays verbatim with parens intact, and `existence_uncertain` flips True instead.
- **`is_dynasty_marker: bool`** — True for the 8 rows whose `name` is a dynasty header (`"0. Dynastie"`, `"7. Dynastie"`, etc.) rather than a person. Authoritative set: `test_179_dynasty_markers_canonical_set`.
- **`is_anti_king: bool`** — True for 4 rows where Beckerath flags the king as Gegenkönig (singular) in `notes_from_beckerath`: 02.09 + 02.10 (Dyn 2 Seth Per-ib-sen and Hor-Seth Cha-sechemui sharing "Gegenkönig der 3 vorigen"), 29.03 (Psamuthis), 31.04 (Chababasch). 11.07 Ment-hotpe IV. is NOT included even though his note contains "Gegenkönige" (plural = "had anti-kings besides him"). 22.07 (Schoschenq IIIa.) is NOT included — its parens-wrap is an existence-uncertainty marker, not an anti-king marker. Authoritative set: `test_179_anti_king_canonical_set`.
- **`existence_uncertain: bool`** — True for 3 rows with explicit uncertainty markers (`(?)`, leading `?`, German quotes `„...“`). Authoritative set: `test_179_existence_uncertain_canonical_set`.

The legacy `egyptian_titulary` + `egyptian_titulary_kind` scalar fields are KEPT alongside the new list as derivative ingest artifacts — they're the read-side source-of-truth that `fix_rows.py` derives from on every run, so dropping them would make subsequent runs read None and silently destroy the typed data. Consumers must use `egyptian_titularies`; the legacy scalars exist solely to keep `fix_rows.py` idempotent.
