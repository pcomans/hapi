...
- **Stage_suffix semantic overload (deferred schema work):** Leprohon's printed `Na.` sub-headword convention is reused in chapter X for queen-consorts (Arsinoe II, Berenike II, Cleopatra I, Cleopatra II) — distinct persons, not titulary stages of the preceding king. The agents emitted these as separate rows with `stage_suffix: "a"` mirroring the literal headword pattern, with a `source_note` flagging the consort-queen semantics. The README's `stage_suffix` definition (lines 122 of README.md, "the SAME king adopting successively-different titulary sets") is therefore overloaded across two unrelated phenomena. Egyptologist-reviewer 2026-04-21 flagged this as P1 — deferred to a future schema-design PR that introduces a `consort_of` or `row_type` field. Until then, downstream consumers must treat `stage_suffix != null` as covering both meanings.
