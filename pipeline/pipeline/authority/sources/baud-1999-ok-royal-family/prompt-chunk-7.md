# Extraction prompt — Baud 1999 BdE 126 Corpus, chunk 7 (entries `[241]`–`[282]`)
...
`pipeline/pipeline/authority/sources/baud-1999-ok-royal-family/raw/source-chunk-7.pdf` — a 31-page sub-PDF of Baud vol. 2 covering physical pages 214–244 of the original. Contents:

- **Physical p. 214** — contains the header block of `[241]`. Your first row is `[241]`.
- **Physical pp. 214–244** — Corpus entries `[241]` through `[282]`. This is the FINAL Corpus chunk; Baud numbers his prosopography `[1]`–`[282]` and what follows is the appendices (out of scope).

**Boundary warning.** There is no entry `[283]` — the Corpus ends at `[282]` on physical p. 244. Pages 245+ are appendix A (cross-references Baud excludes) and are NOT in this sub-PDF. Your last row is `[282]`.
...
- **`source_citation`**: `{"source": "Baud 1999 BdE 126 Corpus [<N>]", "pdf_pages": "214-244", "edition": "IFAO 1999 vol. 2"}`. Every chunk-7 row uses `"214-244"`.
...
- **Do NOT extract `[240]` or `[283]`+.** Your sub-PDF starts on p. 214 with `[241]` and ends on p. 244 with `[282]` (the final Corpus entry). Any row with `baud_id < 241` or `baud_id >= 283` is a bug. Letter-suffixed sub-entries in range `[241a]`..`[282z]` are legitimate.
...
Write newline-delimited JSON to `pipeline/pipeline/authority/sources/baud-1999-ok-royal-family/raw/agent-<YOUR_TAG>-chunk-7.jsonl`, where `<YOUR_TAG>` is `a`, `b`, or `c` (the spawner will tell you which). One row per Baud Corpus entry, in ascending `baud_id` order.
...
**Expected row count:** 42 rows (entries `[241]`–`[282]`), plus any `[Na]`-style sub-entries. If your extraction produces < 35 or > 50, re-read the prompt and the boundary warnings; something is off.
