# Extraction prompt — Baud 1999 BdE 126 Corpus, chunk 6 (entries `[201]`–`[240]`)
...
`pipeline/pipeline/authority/sources/baud-1999-ok-royal-family/raw/source-chunk-6.pdf` — a 35-page sub-PDF of Baud vol. 2 covering physical pages 179–213 of the original. Contents:

- **Physical p. 179** (overlap with chunk 5) — contains the tail of `[200]` and the header block of `[201]`. **Do NOT emit a row for `[200]`** (chunk 5 already emitted). Your first row is `[201]`.
- **Physical pp. 179–213** — Corpus entries `[201]` through `[240]`.

**Boundary warning.** Entry `[241]` begins on physical p. 214 (not in this sub-PDF). **Do NOT emit a row for `[241]`.** Your last row is `[240]`.
...
- **`source_citation`**: `{"source": "Baud 1999 BdE 126 Corpus [<N>]", "pdf_pages": "179-213", "edition": "IFAO 1999 vol. 2"}`. Every chunk-6 row uses `"179-213"`.
...
- **Do NOT extract `[200]` or `[241]`+.** Your sub-PDF starts on p. 179 (containing `[200]`'s tail) with `[201]` starting at the bottom, and ends on p. 213 with `[240]`. `[241]` is on p. 214, not in this sub-PDF. Any row with `baud_id == 200` or `baud_id >= 241` is a bug. Letter-suffixed sub-entries in range `[201a]`..`[240z]` are legitimate.
...
Write newline-delimited JSON to `pipeline/pipeline/authority/sources/baud-1999-ok-royal-family/raw/agent-<YOUR_TAG>-chunk-6.jsonl`, where `<YOUR_TAG>` is `a`, `b`, or `c` (the spawner will tell you which). One row per Baud Corpus entry, in ascending `baud_id` order.
...
**Expected row count:** 40 rows (entries `[201]`–`[240]`), plus any `[Na]`-style sub-entries. If your extraction produces < 35 or > 45, re-read the prompt and the boundary warnings; something is off.
