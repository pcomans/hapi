# Extraction prompt — Baud 1999 BdE 126 Corpus, chunk 5 (entries `[161]`–`[200]`)
...
`pipeline/pipeline/authority/sources/baud-1999-ok-royal-family/raw/source-chunk-5.pdf` — a 38-page sub-PDF of Baud vol. 2 covering physical pages 142–179 of the original. Contents:

- **Physical p. 142** — contains the header block of `[161]`. Your first row is `[161]`.
- **Physical pp. 142–179** — Corpus entries `[161]` through `[200]`.

**Boundary warning.** Entry `[201]` also has a header on physical p. 179 (bottom of page, continuation on p. 180 which is NOT in this sub-PDF). **Do NOT emit a row for `[201]`.** Your last row is `[200]`.

**Large entries.** `[186]` Ḫnt-kꜣw.s Iʳᵉ and `[187]` Nj-wsr-Rꜥ (king Niouserrê himself, as a biographical figure) are unusually long — multi-page narratives with dense footnotes. Parse TITRES carefully.

**Sub-entry warning.** Chunks 2-4 surfaced `[60a]`, `[94b]`, `[101a]`, `[126a]`, `[133b]`, `[139a]`. Watch for `[Na]`-style entries. Emit with `baud_id = "baud-<N><letter>"`.
...
- **`source_citation`**: `{"source": "Baud 1999 BdE 126 Corpus [<N>]", "pdf_pages": "142-179", "edition": "IFAO 1999 vol. 2"}`. Every chunk-5 row uses `"142-179"`.
...
- **Do NOT extract `[201]` or beyond.** Your sub-PDF starts on p. 142 with `[161]` and ends on p. 179 with `[200]`'s content ending mid-page; `[201]` begins at the bottom of p. 179 but continues on p. 180 (not in this sub-PDF). Any row with `baud_id < 161` or `baud_id >= 201` is a bug. Letter-suffixed sub-entries in range `[161a]`..`[200z]` are legitimate.
...
- **Corpus entries are alphabetical by Egyptian transliteration, not by dynasty.** Chunk 5 contains a mix of Dyn 3, 4, 5, and 6 entries.
...
Write newline-delimited JSON to `pipeline/pipeline/authority/sources/baud-1999-ok-royal-family/raw/agent-<YOUR_TAG>-chunk-5.jsonl`, where `<YOUR_TAG>` is `a`, `b`, or `c` (the spawner will tell you which). One row per Baud Corpus entry, in ascending `baud_id` order.
...
**Expected row count:** 40 rows (entries `[161]`–`[200]`), plus any `[Na]`-style sub-entries. If your extraction produces < 35 or > 45, re-read the prompt and the boundary warnings; something is off.
