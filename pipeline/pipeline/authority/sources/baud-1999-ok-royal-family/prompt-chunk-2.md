# Extraction prompt — Baud 1999 BdE 126 Corpus, chunk 2 (entries `[41]`–`[80]`)
...
`pipeline/pipeline/authority/sources/baud-1999-ok-royal-family/raw/source-chunk-2.pdf` — a 34-page sub-PDF of Baud vol. 2 covering physical pages 49–82 of the original. Contents:

- **Physical p. 49** (overlap with chunk 1) — contains the tail of entry `[40]` and the header block of `[41]`. **Do NOT emit a row for `[40]`** (chunk 1 already emitted it). Your first row is `[41]`.
- **Physical pp. 49–82** — Corpus entries `[41]` through `[80]`.

**Boundary warning.** Entry `[81]` (`Mrjj-Ttj Mrj`) has a header at the bottom of physical p. 82 that continues onto p. 83 (not included in the sub-PDF). **Do NOT emit a row for `[81]`.** Your last row is `[80]` (`Mrjj-Ḫwfw*`).
...
- **`roles`**: normalised OK-appropriate role codes derived from `titles_from_baud` and Baud's filiation prose. Controlled vocabulary (extended for chunk 2):
  ...
  - `king's eldest son of his body` (`zꜣ nswt smsw n ẖt.f` or variants with `mrjj.f`)
  - `vizier` (`ṯꜣtj`)
...
- **`source_citation`**: `{"source": "Baud 1999 BdE 126 Corpus [<N>]", "pdf_pages": "49-82", "edition": "IFAO 1999 vol. 2"}`. Every chunk-2 row uses `"49-82"`.
