...
| (merged) | Ch 2 *The Head of the South* — Brief Lives + Unplaced (Dyn 11 transition) | 88–89 | 81–82 | 13 (12 placed + 1 Unplaced) |
| (merged) | Ch 2 *Seizers of the Two Lands* — Brief Lives + Unplaced (Dyn 12 MK proper) | 96–99 | 88–91 | 48 (45 placed + 3 Unplaced) |
...
- **`alt_names`** = list of bracketed variant names Dodson & Hilton record inline (e.g. `Hatshepsut-Khnemetamun` for `Hatshepsut D`). Empty list when absent. **Reserved for alternate forms of the SAME individual's verified name** — NOT for D&H's hedged identity hints across rows (e.g. `Sitre A` "may previously have borne the name Tia (Q)" / `Thutmose B` "conceivably identical with Thutmose Q"). Those hedged cross-references were carried in the now-dropped `notes` prose and are deliberately not promoted to `alt_names`; they remain Phase-A / human-review items. Issue #175 fix.
- **`is_group_entry`** (issue #175 fix) = `bool`. True iff D&H uses letter-range notation in the entry header to cover multiple individuals in a single Brief Lives entry (e.g. `[...]18A–H` covers up to 8 daughters A through H of Amenhotep III; `[...]18K–N` covers 4 daughters K–N). False on every other row including individual lacuna entries (`[...]12A`, `Henut[...]`) which represent ONE partially-attested individual. The canonical set lives in `fix_rows.GROUP_ENTRY_DH_IDS`; `test_is_group_entry_matches_canonical_set` enforces the closure. Phase-A consumers iterating per-row should treat True-flagged rows as multi-individual aggregates.
- **`roles`** = list of Dodson & Hilton's own role-code abbreviations from the parentheses after the name (e.g. `"(KM; KGW; KSis)"` → `["KM", "KGW", "KSis"]`). Extracted verbatim; Phase A expands the codes. **Canonical token set lives in `fix_rows.KNOWN_ROLE_TOKENS`** (closure-tested by `test_role_tokens_in_known_vocab`); the inventory below is illustrative and may drift — when in doubt, consult the constant. Codes seen across all three chunks: `KM` (King's Mother), `KW` (King's Wife), `KGW` (King's Great Wife), `GW` (Great Royal Wife), `KSis` (King's Sister), `KD` (King's Daughter), `KDB` (King's Daughter of his Body), `KSon` (King's Son), `KSonB` (King's Son of his Body — explicit), `KSonK`, `KSonN`, `EKSon` (Eldest King's Son), `EKSonB`, `1KSon`, `1KSonB` (First King's Son of his Body — leading-digit form), `1Genmo`, `HPH` (High Priest of Heliopolis), `HPM` (High Priest of Memphis), `HPA` (High Priest of Amun), `Ador` (Adoratrix), `Exec` (Executive), `ExecH2L` (Executive of the Two Lands), `SPP` (Sem Priest of Ptah), `Genmo` (General), `Gen`, `UWC`, `L2L` (Lady of the Two Lands), `M2L` (Mistress of the Two Lands), `MULE` (D&H's legend code), `MH`, `MoH` (Master of Horse), `GM` (God's Mother), `GWA` (God's Wife of Amun), `GBW` (Greatly Beloved Wife), `ChA`, `2PA`, `1PMut`, `GF`, `Viz` (Vizier), `King of Mitanni`, `King of Hittites`, `Fanbearer`, `Overseer of Treasurers`, `Troop Commander`, `Adjutant of the Chariotry`, `Songstress of Pre`, `Sister of KGW`, `Steward of Queen Tiye A/Tey`, `Nurse of the God` (long-form). Head-of-South chunk adds: `PH` (Priestess of Hathor — the Mentuhotep-II-era Deir el-Bahari mortuary-chapel wives carry it; Phase-A decoding), `GS` (on Tem, wife of Mentuhotep II), `Nomarch` (non-royal provincial governor — on Inyotef A only), and the hedged-role form `KW?` (on possible-wife entries Kawit and Kemsit where D&H writes `(PH; KW?)` — the `?` is preserved as part of the code string). Seizers chunk reuses `UWC` heavily on Dyn-12 wives (Aat, Hetepti, Itaweret, Khnemet, Neferhenut, Sithathoriunet, `[...]12A` — already seen on `Hatshepsut D` in the Power chunk, so not a new code, just a new density profile). Codes genuinely new to this chunk: `GF` (on Senwosret A, probable father of Amenemhat I; sole male non-royal role in the chunk), and the long-form role-phrase `Mistress of All Women` (on Kaneferu — preserved as a single verbatim token inside the `roles` list rather than split on spaces; same treatment as `King of Hittites` / `King of Mitanni` in earlier chunks). Kings and Commoners chunk adds one short token (`RO` — Royal Ornament, on `Inyotef C` only) plus a cluster of long-form spelled-out non-royal administrative role phrases preserved as single verbatim tokens (same treatment as the Ramesside and Seizers long-form tokens): `Governor of El-Kab`, `Overseer of the Fields`, `Chief Scribe of the Vizier`, `Elder of the Portal`, `High Steward`, `Attendant of Dog-Keepers`, `Townsman`, `Royal Representative`. The 13th-Dynasty chapter's extended-family prosopography introduces many non-royal commoners whose civic roles had no short-code equivalent in D&H's legend, producing the density of long-form tokens. Founders chunk adds five Early-Dynastic-specific short tokens, all on female entries: `CTL` (on Herneith, Neithhotep A — likely 'Consort of the Two Ladies' per egyptologist-reviewer PR #79), `FW` (on Herneith, Meryetneith A, Neithhotep A — likely 'First Wife' / 'Foremost Wife' per egyptologist-reviewer), `SH` (on Hotephirnebty, Semat, Seshemetka, `[...]1A` — decoding uncertain without D&H's legend page; egyptologist-reviewer flagged `Stela Holder` / `Subsidiary-Harem` as candidates but could not disambiguate), `SCH` (on Nakhtneith), `ScH` (on Seshemetka). The `SCH`/`ScH` casing split across adjacent entries is preserved verbatim per rule 7 (authority list, never hardcoded) — whether D&H's two casings decode to the same code or two distinct codes is Phase-A work. No authoritative Phase-0 claim is made. The book's abbreviation key lives on pp. 24–37 (front matter); decoding is Phase A work.
...
- **`children_names`** = list of named children mentioned in the prose of this entry (e.g. for Mutemwia: `["Amenhotep III"]`). Empty list when the entry's own prose names no children.
  ...
  **Hedge handling on inferred values.** The Shuttarna II / Tushratta / Hattusilis III cases all involve unhedged child/spouse references in the other entry. When the other entry's statement is hedged (e.g. Hori A's "probably... son of Khaemwaset C"), the inferred `children_names` entry is the bare name; hedges live on the parent-side `father_name`/`mother_name` fields on the child's own row, not on `children_names` lists. This matches the chunk-2 precedent and keeps the lists parseable.
  ...
  Rationale for adopting rather than excluding the rule: D&H's alphabetical-by-individual layout occasionally states the relationship on only one side of a pair, and dropping the cross-inference would produce asymmetric family trees where one entry knows a relationship and its symmetric counterpart does not. Downstream Phase A consumers can rebuild symmetric trees either way; populating both sides in the extract avoids requiring a post-processing "invert relationships" pass. Cross-entry inferences remain traceable because each inferred kinship is independently attested in the counterparty's entry (the structured kinship fields), with the reviewer rationale recorded in `merge-disagreements.txt`.
- **`dynasty`** = integer. `18` for Pre-Amarna and Amarna chunks; `19` for House-of-Ramesses and Feud chunks; `20` for Decline-of-Ramessides (including its Unplaced sub-block, whose heading is `"in 19th and 20th Dynasties"` but whose primary dynasty is 20 unless individual row prose anchors it to 19); `11` for Head-of-South rows (the 11th Dynasty transition at the 1IP/MK boundary — D&H groups the entire dynasty under this sub-section); `12` for Seizers-of-the-Two-Lands rows (the full 12th Dynasty, Amenemhat I → Sobkneferu, including ancestor rows like Senwosret A and Neferet I that arguably straddle late Dyn 11 / early Dyn 12 — D&H's sub-section placement is authoritative); `13` for Kings-and-Commoners rows (13th Dynasty, the start of the Second Intermediate Period — Sobkhotep / Neferhotep / Iy lineages plus extended-family commoners and the 17-row Unplaced sub-block of sisters/sons/daughters/wives of unknown kings); `1` for The-Founders rows (Early Dynastic — D&H's section title joins the 1st, 2nd and 3rd Dynasties into one sub-section, and the per-individual dynasty refinement was applied via `FOUNDERS_CORRECTIONS` in `fix_rows.py` from D&H's per-entry text — e.g. `Shepsetipet` "2nd Dynasty", `Redji` "3rd Dynasty" — which is no longer reproduced verbatim).
- **`sub_period`** = D&H's own subsection title within the chapter, exactly one of: `"The Power and the Glory"` (Pre-Amarna, pp. 137–141), `"The Amarna Interlude"` (pp. 154–157), `"The House of Ramesses"` (pp. 170–175), `"The Feud of the Ramessides"` (pp. 182–183), `"The Decline of the Ramessides"` (pp. 192–194 including Unplaced sub-block), `"The Head of the South"` (pp. 88–89 including Unplaced sub-block), `"Seizers of the Two Lands"` (pp. 96–99 including Unplaced sub-block), `"Kings and Commoners"` (pp. 108–113 including Unplaced sub-block), `"The Founders"` (pp. 48–49 including Unplaced sub-block).
- **`unplaced`** = `true` if the entry sits under D&H's own `Unplaced` sub-section (printed p. 141 for the Pre-Amarna chunk; printed p. 194 for the Decline chunk; printed p. 89 for the Head-of-South chunk — a single entry `Neferkayet`; printed p. 99 for the Seizers chunk — three entries `Didit`, `Neferet Q`, `Sithathor Q`, all sisters-of-unknown-kings; printed p. 113 for the Kings-and-Commoners chunk — seventeen entries `Ahhotepti` through `[...]djeb`; printed p. 49 for the Founders chunk — eleven Early-Dynastic entries `Khnemetptah`, `Menehpet`, `Mesenka`, `Neithhotep B`, `Nysuheqat`, `Qaienneith`, `Redji`, `Shepsetipet`, `Sitba`, `Syhefernerer`, `Wadjetefni`). All other rows: `false`. These are individuals D&H flag as attested but not confidently placed in the family tree — Phase A consumers may want to apply a lower confidence score. Not every Unplaced entry carries a `Q`-suffix disambiguator; the flag follows the section heading in the book, not the name suffix. Conversely, a `Q`-suffixed `dh_id` is not automatically `unplaced: true` — e.g. `Henttawy Q` in the Decline Brief Lives is in the main placed alphabetical run despite the `Q`.
- **`notes`** = **DROPPED.** This field formerly held D&H's full Brief-Lives
  prose paragraph verbatim — their protectable expression. It is removed by the
...
Per ADR-017 (Claude Code subagent OCR → 3 parallel extraction subagents → deterministic merge → egyptologist-reviewer LLM pass → `fix_rows.py` for deterministic post-processing and spot corrections → `destructure_notes.py` to drop the verbatim D&H prose). See `transcribe.md` for the specifics.

**Pipeline order** is `merge.py → fix_rows.py → destructure_notes.py`. The
terminal `destructure_notes.py` stage is deterministic and idempotent: it strips
the verbatim-prose `notes` field from `reconciled.jsonl` and the verbatim
`notes:` lines from `merge-disagreements.txt`. It is a pure function of those
committed artifacts and needs no raw extraction inputs, so it can be re-applied
at any time. `fix_rows.py` carries no `notes`-field corrections (they would
re-introduce prose on a re-run); running `fix_rows.py` after `destructure_notes.py`
...
- **Hedge-suffix normalisation is Phase-A work.** The Seizers chunk's egyptologist review (PR #77) flagged that hedge strings now appear on all three kinship fields — `father_name` (e.g. `"Merenptah (probable)"`), `spouse_names` (e.g. `["Mentuhotep II (possibly)"]`), and `children_names` (e.g. `"Amenemhat I (probable)"` on `Neferet I` and `Senwosret A`). D&H uses four distinct hedge-suffix forms across the corpus — `(probable)`, `(probably)`, `(possible)`, `(possibly)` — reflecting different degrees of authorial uncertainty. Phase A's authority-matcher must strip any of the four suffixes before attempting resolution; Phase-0 extraction preserves all four forms verbatim per rule 1. A single normalisation helper in the enrich stage covers all three fields and all four suffix variants — no prompt or schema change needed.
- **Cross-source duplicates with pharaoh.se (kings who are also D&H entries).** Several D&H Brief Lives entries refer to individuals who are also canonical kings in pharaoh.se: `Sobkneferu` (dh_id scoped to Dyn 12, also a full pharaoh.se ruler as "later female king"), `Ameny A` (explicit prose "later king as AMENEMHAT II" — pharaoh.se has Amenemhat II), `Hatshepsut D` (Power chunk — pharaoh.se has Hatshepsut), `Tausret` (Feud chunk — pharaoh.se has Tausret), etc. Phase-A enrich-stage merging must treat these as **same-individual cross-source records**, not distinct people. The "later king" signal that D&H's prose carried (e.g. `"later female king"`, `"later king as AMENEMHAT II"`) is no longer reproduced now that `notes` is dropped; Phase A resolves these cross-source identities from names + dynasty + the matcher's `same_entity_as` step (ADR-018), not from prose.
- **`alt_names` gap for museum-catalogue matching.** D&H's `dh_id` uses a single canonical spelling per individual (`Mertseger`, `Sithathoriunet`, `Neferitatjenen`, `Inyotef` etc.), but Met / Brooklyn / BM catalogues use variant transliterations heavily (`Meresger`/`Meretseger` for Mertseger; `Intef`/`Antef` for Inyotef; `Nefertatjenen`/`Nefertitotenen` for Neferitatjenen; `Ashait`/`Kauit`/`Kemsyt`/`Henhenit`/`Sadeh` for the Head-of-South Deir el-Bahari wives). This extract emits `alt_names: []` on every row because D&H's own prose never inlines alternative spellings. Phase A must backfill `alt_names` from pharaoh.se / Wikidata / museum-API aggregate data before running the authority-resolution enrich stage — otherwise the cross-source match rate will be artificially low.
