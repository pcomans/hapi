...
- **`notes`** — the full prose paragraph for the entry, verbatim, single-line-joined. Preserve every museum location name (Cairo Museum, Louvre, Turin Museum, British Museum, Brussels, Ny Carlsberg, etc.), tomb identifier (e.g. `Saqqara SNNNN`, `tomb NNN HN at Helwan`, `Umm el-Qaab tomb LETTER`, `BNN at Umm el-Qaab`, `KN at Beit Khallaf`, or any similar coded tomb reference that appears in the entry's prose), stela / statue / block reference (whether numbered — `stela (number NN)` — or named — e.g. `Cairo Annals Stone`, `Palermo Stone`, `Royal Tomb at Naqada`, `Step Pyramid`), named royal monument, and markdown footnote marker (e.g. `[^NN]`). Preserve D&H's hedges (`probable`, `probably`, `possible`, `possibly`, `perhaps`, `conceivably`, `presumably`) as part of the surrounding prose. Trim leading / trailing whitespace.
