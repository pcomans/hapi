...
- **`notes`** — the full prose paragraph for the entry, verbatim, single-line-joined. Preserve museum locations (Cairo Museum / CM, British Museum, Metropolitan Museum, Louvre, Würzburg, Vatican, Vienna, Basel, Tukh, Kerma, Boston, Bologna, Rio de Janeiro, etc.), named papyri (`Papyrus Bulaq 18`, `Juridical Stela from Karnak`, `Kahun papyrus`, `Turin Canon`), tomb IDs (`tomb of Reniseneb B`, `tomb 9 at El-Kab`), inscription locations (`Wadi Hammamat`, `Wadi el-Hol`, `Wadi el-Hudi`, `Philae`, `Sehel`, `Karnak`, `Abydos`, `Koptos`), and D&H's hedges (`probable`, `probably`, `possible`, `possibly`, `perhaps`, `conceivably`). Trim leading / trailing whitespace. Do NOT summarise, editorialise, or add scope commentary.
   ...
   **Hetepti cross-reference stub.** The chunk entry for `Hetepti` (KM; M2L; UWC) has a single-line notes body: `"See previous section."`. Preserve it verbatim — this is the cross-section-duplicate pattern (same individual, full Brief Life in the Seizers of the Two Lands chunk). `spouse_names`, `father_name`, `mother_name`, `children_names` are all `null`/`[]` on this stub — the stub doesn't repeat the kinship clauses.
...
- **Cross-section duplicate stub on Hetepti**: emit a row for Hetepti in this chunk with `dh_id: "Hetepti"`, `roles: ["KM", "M2L", "UWC"]`, `notes: "See previous section."`, `spouse_names: []`, `father_name: null`, `mother_name: null`, `children_names: []`, `sex: "female"` (from the role codes), `unplaced: false`. The full Brief Life for Hetepti is in the Seizers of the Two Lands chunk under a different `sub_period`. Composite `(dh_id, sub_period)` key handles both rows independently.
