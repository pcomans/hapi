...
**Method deviation.** Earlier DH chunks used an intermediate OCR'd `chunk-*.md` file produced by a Gemini OCR subagent. This chunk uses a different path: the source pages were split to a small standalone sub-PDF (`raw/source-p190-p194.pdf`, ~2.4 MB, 5 pages, SHA-256 pinned in `transcribe.md`) that the extraction subagent reads **directly via the `Read` tool** with `pages: "1-5"`. The Read tool's PDF renderer is deterministic and serves all three agents identical page images. This bypasses the Opus-OCR-refusal-then-Gemini-fallback dance documented in `transcribe.md` and is acceptable per the playbook's "Method deviation" provision because the printed source remains the ground truth and the sub-PDF is hash-pinned for audit.

**Trade-off vs Gemini-OCR path.** The OCR-to-md pipeline (used by chunks 1–7) produces an intermediate `chunk-*.md` artefact that is independently auditable / diffable; the direct-PDF-Read pipeline loses that intermediate-artefact diff layer and instead relies on (a) the SHA-pinned sub-PDF as the audit anchor and (b) a committed `dh_id_corrections-<chunk>.json` + `pre_merge.py` runner to canonicalize the small OCR-drift mismatches that occur on bold-italic headword styling. Both pipelines preserve the printed-source-page-image redundancy across all 3 agents; the OCR-to-md path adds a 2nd-pass diff layer that this path drops. Documented in `transcribe.md` as a sanctioned per-chunk method choice.
...
- **`dh_id`** — D&H's bold name-with-disambiguator exactly as printed. Roman-numeral suffixes (`I`, `II`, `III`, ...), Latin-letter suffixes (`A`, `B`, `C`, ...), and the special `Q`-suffix (D&H uses `Q` for queens whose primary identification is via spousal role) are all part of the ID — preserve them as printed. Compound-name forms are part of the ID too (e.g. parenthetical prefix forms like `(Prefix-)Name`).
  ...
  **Cross-section duplicates with the preceding Decline-of-the-Ramessides chunk.** Several Dyn-21 individuals (King's Mothers / King's Wives whose roles bridge the late Dyn-20 / early Dyn-21 transition) appear in BOTH this Of Kings and Priests Brief Lives AND in the Decline of the Ramessides Brief Lives (where D&H prints a stub `(see next section)` cross-reference). The composite `(dh_id, sub_period)` key in this source's schema handles this — emit the Dyn-21 row here with `sub_period: "Of Kings and Priests"` per the established cross-section-duplicate rule. The Decline-side stub rows are already in `reconciled.jsonl` from prior chunks. If you observe any apparent within-chunk duplicate (same dh_id appearing twice in your own output for this chunk), flag it in your final report.
