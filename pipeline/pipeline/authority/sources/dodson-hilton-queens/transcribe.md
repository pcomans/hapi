...
The chunk committed to the pipeline was therefore produced by **Google Gemini** via a one-shot prompt (the exact prompt text lives in this repo as prior-conversation record; it describes the three-column Brief Lives format, role-code preservation rules, skip-list for photo captions, and output conventions). Gemini's transcription was spot-checked against the PDF pages (visually, in the Claude Code main session) and is consistent with what's printed on pp. 137–141.

This is a one-off workaround for this chunk. The downstream pipeline (3-subagent extraction, merge, reviewer pass, fix_rows) continues unchanged — those stages operate on Markdown input and do not hit the same filter. Follow-up Dodson-Hilton PRs (Amarna, Ramesside) will re-evaluate whether Opus has become viable for D&H OCR or whether Gemini remains the fallback.
