...
Per ADR-017 and `docs/playbook-phase-0-ocr-transcription.md`. Five chunks landed so far (Power-and-Glory, Amarna Interlude, Ramesside sub-blocks, Head of the South, Seizers of the Two Lands); further chunks (remaining earlier chapters) follow the same method.
...
The offset continues to drift (incrementally, through the 2IP Brief Lives sub-block yet to be extracted) before reaching the Ch 3 `+11`. Each Ch 2 follow-up chunk must re-verify the offset at its own boundaries.
