# Extraction prompt — Porter & Moss Vol III.1 (Memphis), Chunk 11a

> **Chunk 11a (first half of Steindorff Cemetery)** — covers `STEINDORFF CEMETERY` D-numbered tombs D.I through D.38 + interstitial named tombs (WEMTETKA, IBIR). Physical pp.105–108 / printed pp.108–111. Chunk 11b will pick up at D.39/40 → D.221. Split into halves after a 6-agent infrastructure failure on the full 40-row chunk. **D-numbered tombs are PRIMARY** here: PM prints `D. <NUM>. <NAME IN CAPS> <Title cluster>. <Dating>.` as the standard headword form, with `D. <NUM>` being Steindorff's own field numbering (NOT Mariette's Saqqâra D-prefix).

You are one of three independent extraction subagents. Read the text-layer chunk file at `pipeline/pipeline/authority/sources/porter-moss-memphis/raw/chunk-11a-p105-p108-steindorff-D1-D38.txt` and produce a JSONL file with one structured row per **PM-headworded mastaba** in the Steindorff Cemetery cluster, D.I through D.38 + interstitial named tombs. The other two agents see the same prompt and the same chunk; their outputs are majority-voted by `merge.py`.
...
- `pipeline/pipeline/authority/sources/porter-moss-memphis/raw/agent-a-chunk11a.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-memphis/raw/agent-b-chunk11a.jsonl`
- `pipeline/pipeline/authority/sources/porter-moss-memphis/raw/agent-c-chunk11a.jsonl`
...
Pre-extraction structural scan for chunk 11a (D.I → D.38 + interstitials): PM prints ~13–15 D-numbered headworded entries (mostly Shape-1 named, two bare Shape-2 at D.I and D.38) + ~2 interstitial Shape-5 named tombs (WEMTETKA, IBIR — appearing between D-numbered entries with no D-prefix line). **Total expected: ~15–17 rows (acceptable band 13–20).** If your final count is below 12 or above 22, re-read the chunk file — you've either missed tombs or emitted out-of-scope body items / chapel sub-features.
...
```
agent-<X>-chunk11a: <count> rows; <shape1>/<shape2>/<shape3>/<shape4>/<shape5> split; <anomalies or "none">
```
