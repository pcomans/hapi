...
- Bare-headword shafts (`G 7112.`, `G 7142.`) emit `occupant_name: null`, `occupant_role: "Unknown"`, `attribution_certainty: "uncertain"` rows. This required diverging from the Theban-source `merge.SENTINEL_NULL_STRINGS` — removing `"unknown"` from that frozenset so `"Unknown"` survives as a controlled-vocab value rather than collapsing to `None`.
