...
- **"good name" alt-name idiom standardisation.** Egyptian *rn nfr* "beautiful name" — the secondary personal name many OK officials used alongside their primary name. PM prints `<PRIMARY> good name <ALT>` in the headword. Chunk-8 codifies the convention: primary in `occupant_name`, alt in `occupant_alt_names` as a single entry. Examples (data-internal references only — verify in the printed source p.85, p.87): SENEZEMIB INTI, SENEZEMIB MEḤI, MERYREʿ-MERYPTAḤʿANKH NEKHEBU, RUZ HAGI, NIMESTY (anonymous wife).
