...
- **`substream: str | None`** — extracted from the kitchen_id letter (`H` = Theban HPA, `E` = Sais-Bubastis-East, `P` = Persian / proto-Saite, `None` = main-line dynasty). Replaces inferring the substream from string-prefix scrutiny on `kitchen_id`. Closure-tested for shape (`test_180_substream_derives_from_kitchen_id`) and distribution (42 main-line, 10 H, 4 E, 4 P).
...
- **`is_co_regent_only: bool`** — True for rows that catalogue an HPA co-regent rather than a sole ruler (21H.05 Masaharta, 21H.06 Djed-Khons-ef-ankh).
- **`existence_doubtful: bool`** — True when Kitchen marks the king's existence as uncertain via `?` / `??` on the name OR by wrapping the entire name in single quotes. Pinned set: `{24E.01, 24E.02, 24P.01}`.
- **`same_person_as: str | None`** — kitchen_id of the row this row's king is also catalogued as. Pinudjem I appears at 21H.03 (HPA capacity) and 21H.04 (king titulature) — same person, two table rows. Closure-tested for symmetry (each row points back at the other).
