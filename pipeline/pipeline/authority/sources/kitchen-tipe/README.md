...
- `prenomen` = Kitchen's anglicised prenomen when the table gives one (format `"{name}, {prenomen} (…)"`); null otherwise. Preserve Kitchen's own punctuation. **Exception — `[Prenomen unknown]` is not a prenomen** and never appears in this field: it is Kitchen stating that the throne name is not recorded, and it lives in `prenomen_absence` (below) with the printed token retained verbatim.
...
## Issue #180 schema additions (2026-05-03 audit-fix)

The audit-fix adds typed flags + a typed prenomen list to disambiguate the multiple semantics overloaded onto null fields and in-string sentinels. Every row carries every field after `fix_rows.py` runs (closure-tested by `test_180_every_row_has_typed_flags`).

- **`substream: str | None`** — extracted from the kitchen_id letter (`H` = Theban HPA, `E` = Early Saite Princes / Chief-of-Mā, `P` = proto-Saite Dyn 24 successors / Tefnakht-line predecessors, `None` = main-line dynasty). Per egyptologist P3, the `P` substream does NOT designate Persian (Dyn 27 lies post-525 BCE and is out of TIPE scope). Replaces inferring the substream from string-prefix scrutiny on `kitchen_id`. Closure-tested for shape (`test_180_substream_derives_from_kitchen_id`) and distribution (42 main-line, 10 H, 4 E, 4 P).
- **`prenomen_absence: {kind, printed_as} | null`** — non-null for the 2 rows where Kitchen explicitly prints `[Prenomen unknown]` (22.04 Takeloth I, 23.07 Iuput II) — a positive assertion that the king exists but the value isn't recorded. Distinct from rows where `prenomen` is null *and* `prenomen_absence` is null, which means Kitchen's table layout doesn't include that column for that row (most HPA / E / P rows). `kind` is drawn from the controlled vocabulary in `pipeline/authority/claimgraph/absence.py`; `printed_as` retains Kitchen's exact printed token, so the migration is lossless.

  Supersedes the earlier boolean `prenomen_is_kitchen_unknown`, which was correct as far as it went but left the placeholder string sitting **in** `prenomen` while nothing consulted the flag. The claim-graph loader read that string as a throne name and normalised it into matching keys, so two kings whose throne names are equally UNrecorded could corroborate each other into one person — absence behaving like a fact (Rule 2). The loader now consults `prenomen_absence`, and **raises** if a source ships an absence flag it does not read (`_ABSENCE_CONSULTED` in `claimgraph/sources.py`), which is the failure mode that let the old boolean rot.
- **`is_co_regent_only: bool`** — True for rows that catalogue a co-regent rather than a sole ruler. Two detection sources: (a) literal "co-rgt only" annotation in `notes_from_kitchen` (22.03 Shoshenq II, 22.06 Harsiese), and (b) scholarly-judgment overrides for HPA narrative co-regents whose table cell lacks the marker (21H.05 Masaharta, 21H.06 Djed-Khons-ef-ankh). Pinned set: `{21H.05, 21H.06, 22.03, 22.06}`.
- **`existence_doubtful: bool`** — True when Kitchen marks the king's existence as uncertain. Two detection sources: (a) `?` / `??` on the name OR entire name wrapped in single quotes (24E.01, 24E.02, 24P.01); (b) literal `existence, doubtful` annotation in `notes_from_kitchen` (23.08 Shoshenq VI). Pinned set: `{23.08, 24E.01, 24E.02, 24P.01}`.
- **`same_person_as: str | None`** — kitchen_id of the row this row's king is also catalogued as. Two pairs: 21H.03 ↔ 21H.04 (Pinudjem I HPA capacity ↔ king titulature) and 24E.04 ↔ 24.01 (Tefnakht I Chief-of-Mā ↔ king; Kitchen's "(c. 13 y; then, kg)" chain on 24E.04 explicitly links them). Closure-tested for symmetry.
- **`corrected_end_bce: int | None`** — for 21H.06 only. Kitchen's printed `end_bce=-1056` is a typographic reversal; the corrected value is `-1045` (predecessor Masaharta ends 1046, successor Menkheperre starts 1045, length 1y). Replaces the prior pattern of hardcoding the correction in a `DKF_INTERVAL` Python constant + putting the explanation in `notes_from_kitchen` prose.
- **`prenomens: list[{name, when}]`** — populated only for 25.03 Piankhy: Kitchen's `Usimare, then Sneferre` decomposes to two entries with `when="initial"` / `when="later"`. The legacy `prenomen` scalar still carries the comma-string for backwards compat (and for fix_rows.py idempotence).

The legacy `prenomen` scalar is KEPT alongside the new `prenomens` list as a derivative ingest artifact — re-runs of `fix_rows.py` remain idempotent.
