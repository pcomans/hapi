...
- **`prenomen_is_kitchen_unknown: bool`** — True for the 2 rows where Kitchen explicitly prints `[Prenomen unknown]` (22.04 Takeloth I, 23.07 Iuput II) — a positive assertion that the king exists but the value isn't recorded. Distinct from rows where `prenomen` is null because Kitchen's table layout doesn't include that column for that row (most HPA / E / P rows).
