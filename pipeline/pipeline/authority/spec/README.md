...
Both are CRM-SIG-issued artifacts. Hapi-specific additions on top of the two specs are declared in a separate manifest at `../hapi_extension.rdf`, in three structurally different shapes: (a) `rdfs:subClassOf` / `rdfs:subPropertyOf` narrowing declarations plus `owl:SymmetricProperty` typing on the two symmetric Hapi predicates (the standard CIDOC extension idiom — same as how CRMdig itself extends core CRM); (b) free-standing `rdf:Property` / `rdfs:Class` declarations for project vocabulary that has no clean CRM/CRMdig parent (`hapi:tomb_owner`, `hapi:in_dynastic_period`, `hapi:matcher_review_verdict`, `hapi:supersedes`, etc. — the two burial predicates `hapi:original_burial_in` / `hapi:cache_context_at` are NOT in this bucket: they are subPropertyOf-narrowed under `crm:P53` via the E53 place pairing, see ADR-018) — CIDOC's open extension model permits this shape but the shape-(b) predicates are intentionally CIDOC-opaque (no `subPropertyOf` declaration means no CRM/CRMdig term to interpret through); and (c) controlled-vocabulary `crm:E55_Type` instances declared via `<rdf:Description rdf:about="#..."><rdf:type rdf:resource=".../E55_Type"/>` (currently the four verdict-outcome instances `hapi:verdict_approved` / `_rejected` / `_retracted` / `_escalated` — NOT predicates; CIDOC vocabulary instances typed directly under CIDOC's E55 Type class, used as P141 values). Reader behaviour for Hapi-namespaced triples depends on what the reader has loaded and which reasoner it applies:
